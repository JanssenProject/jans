 ...
 */
public class LdapOperationsService implements PersistenceOperationService {

	private static final Logger log = Logger.getLogger(LdapOperationsService.class);
	...
	private LdapOperationsService() {
	}

	public LdapOperationsService(LdapConnectionProvider connectionProvider) {
		this(connectionProvider, null);
	}

	public LdapOperationsService(LdapConnectionProvider connectionProvider, LdapConnectionProvider bindConnectionProvider) {
	...
	protected String lookupDnByUid(String uid, String baseDN) throws SearchException {
		Filter filter = Filter.createEqualityFilter(LdapOperationsService.uid, uid);
	...
	public boolean addEntry(String dn, Collection<Attribute> atts) throws DuplicateEntryException, ConnectionException {
		try {
			...
			if (result.getResultCode().getName().equalsIgnoreCase(LdapOperationsService.success))
				return true;
	...
	public boolean updateEntry(String dn, Collection<Attribute> attrs) throws DuplicateEntryException, ConnectionException {
		...
		for (Attribute attribute : attrs) {
			...
			if (attribute.getName().equalsIgnoreCase(LdapOperationsService.objectClass)
					|| attribute.getName().equalsIgnoreCase(LdapOperationsService.dn)
					|| attribute.getName().equalsIgnoreCase(LdapOperationsService.userPassword)) {
				continue;
			}
