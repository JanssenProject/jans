...
# Default-Stop:      0 1 6
# Short-Description: oxd-https-extension start script.
# Description:       Start oxd https extension
### END INIT INFO
...
# processname: oxd-https-extension
# ========================
...
#   sequence of shell commands, like setting relevant environment variables.
#
#   The files will be checked for existence before being passed to oxd-https-extension
#
...
then
  echo "** ERROR: Oops! oxd https extension doesn't appear to be installed in $OXD_HOME"
...
get_pid() {
        if [ -f $OXD_PID_FILE ]; then
                ...
                PID_NUM="`ps -eaf|grep -i java|grep -v grep|grep -i 'oxd-https.yml'|awk '{print $2}'`"
...
OXD_ARGS=(${OXD_ARGS[*]} "oxd-https-extension.state=$OXD_STATE")
...
do_start () {
        ...
        if [ "x$PID_NUM" = "x" ]; then
                ...
                then
      			...
      			start-stop-daemon -S -p"$OXD_PID_FILE" $CH_USER -d"$OXD_BASE" -b -m -a "$JAVA" -- "${RUN_ARGS[@]}" >> "$OXD_LOGS/oxd-https.log" 2>&1
