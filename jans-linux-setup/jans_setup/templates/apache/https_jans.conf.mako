...
<VirtualHost *:443>
    ...
    SSLCertificateFile ${httpdCertFn}
    SSLCertificateKeyFile ${httpdKeyFn}
    ...
    <Location /jans-auth>
        Header edit Set-Cookie ^((?!opbs|session_state).*)$ $1;HttpOnly
        ProxyPass http://localhost:${jans_auth_port}/jans-auth retry=5 connectiontimeout=60 timeout=60
    ...
    <Location /jans-casa>
        Header edit Set-Cookie ^((?!opbs|session_state).*)$ $1;HttpOnly
        ProxyPass http://localhost:${casa_web_port}/jans-casa retry=5 connectiontimeout=60 timeout=60
        Order deny,allow
        Allow from all 
    </Location>
