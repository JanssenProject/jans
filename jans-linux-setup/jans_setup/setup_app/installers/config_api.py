...
class ConfigApiInstaller(JettyInstaller):
    ...
    source_files = [
                ...
                (os.path.join(Config.dist_jans_dir, 'cache-refresh-plugin'), os.path.join(base.current_app.app_info['JANS_MAVEN'], 'maven/io/jans/jans-config-api/plugins/cache-refresh-plugin/{0}/cache-refresh-plugin-{0}-distribution.jar').format(base.current_app.app_info['ox_version'])),
                ]
