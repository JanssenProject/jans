...
export default function registerOptionsTests(QUnit: QUnitApi): void {
    ...
    for (const testCase of [
      ...
      assert.throws(
        ...
        (error: unknown) => {
          ...
          return inputError.code === testCase.code &&
            JSON.stringify(inputError.path) === JSON.stringify(testCase.path);
        },
        testCase.path.join("."),
      );
    }
  ...
  QUnit.test("explicit null option values do not select defaults", (assert) => {
    for (const testCase of [
      {
        options: { logging: { type: "memory", level: null } },
        code: "INPUT_UNSUPPORTED",
        path: ["logging", "level"],
      },
      {
        options: { contextStore: { memoryAlertThresholdPercent: null } },
        code: "INPUT_OUT_OF_RANGE",
        path: ["contextStore", "memoryAlertThresholdPercent"],
      },
      {
        options: { issuerLoading: { mode: null } },
        code: "INPUT_UNSUPPORTED",
        path: ["issuerLoading", "mode"],
      },
    ] as const) {
      assert.throws(
        () => prepareCedarlingOptions({
          applicationName: "null-option",
          policyStore: inlinePolicy,
          ...testCase.options,
        } as never),
        (error: unknown) => {
    ...
    for (const testCase of cases) {
      assert.throws(
        ...
        (error: unknown) => {
          ...
          return (
            inputError.code === "INPUT_OUT_OF_RANGE" &&
            JSON.stringify(inputError.path) === JSON.stringify(testCase.path)
          );
        },
        testCase.path.join("."),
