...
class MainActivity : ComponentActivity() {
    /**
     * Initializes the activity UI and launches the Compose content for the app's main screen.
     *
     * Sets up edge-to-edge display and sets the Compose content to a themed Scaffold that fills
     * the screen and hosts the CardListScreen composable.
     */
...
/**
 * Displays the card list UI that loads required assets and lets the user view an authorization state dialog.
 *
 * Loads these assets from the app's assets: "bootstrap.json", "resource.json", "context.json", "tokens.json",
 * "action.txt", and the archived policy store "MyStore.cjar". Renders DataCard entries for any loaded JSON:
 * bootstrap configuration, resource, context, and each token entry. Provides a radio button group to select
 * the log tag and a "Show State" button that opens a modal dialog.
 *
 * When the dialog is opened and both the bootstrap JSON and policy store archive are available, the dialog
 * creates a Cedarling instance, converts tokens to a list of TokenInput, calls authorizeMultiIssuer(...) with
 * the tokens, action, resource, and context, and then retrieves logs for the resulting requestId and selected tag.
 * The dialog displays the authorization result and the retrieved logs via StateDialog.
 */
@Composable
fun CardListScreen() {
