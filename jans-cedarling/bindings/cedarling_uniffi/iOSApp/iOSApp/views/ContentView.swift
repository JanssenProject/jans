...
struct ContentView: View {
    ...
    /// Performs an authorization request with the current UI inputs, loads policy/archive, runs the multi-issuer authorization, fetches logs, and prepares modal content for display.
    /// 
    /// Side effects: updates `resultMessage`, `modelTextField`, `modelJsonField`, `modelJsonLogField`, `isAuthzRequest`, `authzRequestType`, and `showModal` to reflect the outcome.
    /// If the policy archive (`MyStore.cjar`) is missing or unreadable, the function sets `resultMessage` to an explicit error and returns without presenting results. On other errors, `resultMessage` is set to an "Unexpected error occurred" message.
    — Parameters:
    ///   - authzRequestType: Indicates which modal presentation to show for the authorization outcome (e.g., `.result` to show decision details or `.logs` to show request logs).
