...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Average single authorization: {} μs ({:.3} ms)", avg_single, avg_single as f64 / 1000.0);
    ...
    for (i, doc) in documents.iter().enumerate() {
        ...
        if i % 200 == 0 {
            println!("  Processed {} documents: {} μs per auth", i, auth_duration.as_micros());
        }
    }
    ...
    println!("    Average per document: {} μs ({:.3} ms)", avg_per_doc, avg_per_doc as f64 / 1000.0);
    ...
    for batch_size in batch_sizes {
        ...
        println!("  Batch size {}: {:.2} ms total, {} μs per doc ({:.3} ms)", 
                 batch_size, batch_duration.as_millis(), avg_per_doc, avg_per_doc as f64 / 1000.0);
    }
    ...
    let _results: Vec<_> = futures::future::join_all(
        large_batch.iter().map(|doc| cedarling.authorize_unsigned((*doc).clone()))
    ...
    println!("  Our batch result: {:.3}ms per document", avg_per_doc as f64 / 1000.0);
    println!("  Ratio: {:.2}x", (avg_per_doc as f64 / 1000.0) / 2.0);
    
    if avg_per_doc as f64 / 1000.0 > 2.0 {
        println!("  Our result is {:.2}x slower than OpenSearch benchmark", 
                 (avg_per_doc as f64 / 1000.0) / 2.0);
    } else {
        println!("  Our result is {:.2}x faster than OpenSearch benchmark", 
    ...
    for i in 0..count {
        ...
        let principals = vec![EntityData {
            ...
            attributes: HashMap::from([
                ...
                ("email".to_string(), json!(format!("student{}@university.edu", i))),
        ...
        let document = RequestUnsigned {
            ...
            resource: EntityData {
                ...
                attributes: HashMap::from_iter([
                    ("org_id".to_string(), serde_json::Value::String("university_123".to_string())),
                    ("country".to_string(), serde_json::Value::String("US".to_string())),
                    ("name".to_string(), serde_json::Value::String(name)),
                    ("gpa".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(gpa).unwrap())),
                    ("grad_year".to_string(), serde_json::Value::Number(serde_json::Number::from(grad_year))),
                ]),
            },
        };
