...
//!
//! ## Tests Included
//! - Tests for errors when the `userinfo_token` has an invalid signature.
...
fn errors_on_expired_token() {
    ...
    assert!(
        ...
        "Expected decoding to fail due to `userinfo_token` being expired: {:?}",
        decode_result
    );
...
#[test]
/// Tests that [`JwtService::decode_tokens`] returns an error when the `userinfo_token` is used
...
fn errors_on_token_used_before_nbf() {
    ...
    // Invalid userinfo_token claims (nbf has not yet passed)
    let userinfo_token_claims = json!({
        "iss": server.url(),
        "aud": "some_aud".to_string(),
        "sub": "some_sub".to_string(),
