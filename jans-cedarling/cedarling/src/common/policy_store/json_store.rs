...
/// Represents a JSON structure for deserializing a Policy Store.
///
/// This is the Schema of the Policy Store generated by Agama Lab.
#[derive(Deserialize)]
pub struct PolicyStoreJson {
    ...
    /// A map of policy store IDs to their data.
...
impl TryFrom<PolicyStoreJson> for PolicyStore {
    ...
    /// Attempts to convert `PolicyStoreJson` into a `PolicyStore`.
    ///
    /// # Errors
    /// Returns `LoadFromJsonError` if the policy store is empty or contains more than one store.
...
/// Represents the structure of a single policy store in the JSON format.
...
pub struct PolicyStoreDataJson {
    ...
    /// Policies stored as a map with Base64 encoded content.
    #[serde(deserialize_with = "parse_encoded_policies")]
    pub policies: HashMap<String, PolicyContent>,
    /// Cedar schema associated with the policy store.
    #[serde(alias = "schema", deserialize_with = "parse_encoded_schema")]
    pub cedar_schema: CedarSchema,
    /// Trusted issuers for the policy store.
    #[serde(default)]
    pub trusted_issuers: HashMap<String, TrustedIssuerMetadata>,
}

/// Parses a map of Base64-encoded policies from a deserializer.
///
/// # Errors
/// Returns a deserialization error if policies cannot be decoded or parsed.
...
/// Parses a Base64 encoded Cedar schema.
///
/// # Errors
/// Returns a deserialization error if the schema cannot be decoded or loaded.
...
///
/// # Errors
/// Returns a deserialization error if the Base64 string cannot be decoded or converted to UTF-8.
...
/// Represents errors that may occur when loading a Policy Store from JSON.
#[derive(Debug, thiserror::Error)]
pub enum LoadFromJsonError {
    /// Error indicating that no policy store was found.
    #[error("The JSON policy stores does not contain a single policy store.")]
    NoPolicyStore,
    /// Error indicating that more than one policy store was found.
    #[error("The JSON policy stores contain more than one policy store.")]
    MoreThanOnePolicyStore,
    /// Error occurring during deserialization.
