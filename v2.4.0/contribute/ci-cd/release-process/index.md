...
Releases are cut by the `release-trigger.yml` workflow (version bump, tag `v<version>`, GitHub Release), which starts the [build chain](https://docs.jans.io/v2.4.0/contribute/ci-cd/architecture/index.md). Nightly prereleases are produced by `build-nightly.yml`.
