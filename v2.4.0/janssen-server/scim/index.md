...
The API is available as a component of the Janssen Server. Upon [installation](https://docs.jans.io/v2.4.0/janssen-server/install/vm-install/vm-requirements/index.md) you can select if you want SCIM included in your environment. To add SCIM post-install do the following:
...
To know more about OAuth protection mode please visit [here](https://docs.jans.io/v2.4.0/janssen-server/scim/oauth-protection/index.md). The SCIM API endpoints are by default protected by (Bearer) OAuth 2.0 tokens. Depending on the operation, these tokens must have certain scopes for the operations to be authorized. We need a client to get Bearer token.

## API documentation at a glance

[SCIM API](https://docs.jans.io/v2.4.0/janssen-server/reference/openapi/index.md) doc page describes about our implementation of SCIM. The API has also been documented using OpenAPI (swagger) specification for the interested.
...
1. Always check your [logs](https://docs.jans.io/v2.4.0/janssen-server/scim/logs/index.md).
