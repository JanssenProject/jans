...
> A metadata document may also declare `spiffe_id`/`spiffe_bundle_endpoint`, used by [SPIFFE-Based Client Authentication](https://docs.jans.io/v2.4.0/janssen-server/auth-server/oauth-features/spiffe-client-auth/index.md) to bind a CIMD `client_id` to a SPIFFE identity.
