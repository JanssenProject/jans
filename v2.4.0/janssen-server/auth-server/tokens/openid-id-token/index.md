...
The `amr` (Authentication Method References) claim is normally derived from the authentication method (acr) used for the session. When authentication is performed through an [Agama](https://docs.jans.io/v2.4.0/janssen-server/developer/agama/jans-agama-engine/index.md) flow, the flow itself can additionally report the authentication method(s) it enforced by including an `amr` entry in the `data` passed to `Finish`; this value is merged into the `amr` claim of the resulting `id_token`. See [Authentication and `Finish`](https://docs.jans.io/v2.4.0/janssen-server/developer/agama/jans-agama-engine/#authentication-and-finish) for details.
