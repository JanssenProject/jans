...
1. Now we can run `terraform init`, which will fetch the plugins needed.
...
1. Have a `client_id` and `client_secret` with sufficient scopes and permissions.
   ...
   - **K8s:**
     ...
     1. client_secret: `kubectl get secret cn -n <namespace> --template={{.data.test_client_pw}} | base64 -d`

1. In your `variables.tf` file, add your `client_id`, `client_secret` and `FQDN`
...
1. Configure the provider section in `configurations.tf` file:
...
1. Before importing, we have to define a `resource` configuration in `configurations.tf`:
...
1. Import: `terraform import jans_logging_configuration.global global`
...
1. Add configuration to your `configuration.tf file`
...
1. You can run `terraform fmt` to rewrite Terraform configuration files to a canonical format and style.

1. You can validate your Terraform syntax using `terraform validate`

1. Plan and then apply changes using `terraform plan` and `terraform apply` respectivley.

1. Review the changes now using `terraform state show jans_logging_configuration.global`. Now it should show `logging_level = "TRACE"`.

1. We can double-check the `logging level` in the TUI
