...
The minimal configuration required to use the CORS Filter is shown below, and is already added to the `web.xml` in `jans-auth.war`. The filter name is `io.jans.as.server.filter.CorsFilter`.
...
1. Go to `Auth Server > Properties` and select `corsConfigurationFilters`
