...
WebAuthn hints (configured in the [FIDO2 Server Configuration](https://docs.jans.io/v2.4.0/janssen-server/config-guide/fido2-config/janssen-fido2-configuration/index.md)) guide the Relying Party (RP) interface during registration or authentication:
...
[Conditional UI](https://docs.jans.io/v2.4.0/janssen-server/fido/conditional-ui-and-fallback/index.md) allows browsers to autofill username fields with available passkeys. The flow works as follows:
...
- **Script Integration**: Refer to the Python code inside [PasskeyInterceptionScript.py](https://docs.jans.io/v2.4.0/script-catalog/person_authentication/passkey/PasskeyInterceptionScript.py) which handles reading and parsing the `allowList` cookie during authorization.
...
Refer to [FIDO2 configuration documentation](https://docs.jans.io/v2.4.0/janssen-server/config-guide/fido2-config/janssen-fido2-configuration/index.md) for more details.
...
For API specification mappings, refer to the [Config-API OpenAPI Document](https://docs.jans.io/v2.4.0/janssen-server/reference/openapi/index.md).
