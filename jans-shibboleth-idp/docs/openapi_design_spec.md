...
- [x] **Delete** — `DELETE /trust-relationships/{id}` (`deleteTrustRelationship`) — persistence-layer hard delete (D11); no domain operation, no request/response body. `204` / `401` / `404`. *No deletion guard for now (§10).* OpenAPI-only — no DTO/mapper (deletion is a persistence concern).
...
- [x] **Set SAML2 SSO profile** — `PATCH /trust-relationships/{id}/profiles/saml2-sso` — `updateSaml2SsoProfileConfiguration`. Partial-override (D16); the fullest profile — 21 fields (ECP's 19 plus the Authentication capability's `post_authentication_flows` and `max_authentication_age`). Three ISO-8601 durations. Response `TrustRelationshipSummary`; errors `400`/`401`/`404`/`409`. DTO `Saml2SsoProfileConfigurationRequest`; mapper `TrustRelationshipMapper.updateSaml2SsoProfileConfiguration`; tests in [`trust_dto_mapper_tests.md`](./trust_dto_mapper_tests.md).
...
- [x] **Set SAML2 ECP profile** — `PATCH /trust-relationships/{id}/profiles/saml2-ecp` — `updateSaml2EcpProfileConfiguration`. Partial-override (D16); 19 fields — adds the Saml2Sso-capability fields (`authentication_result_reuse_policy`, `assertion_encryption_policy`, `attribute_encryption_policy`, `maximum_sp_session_lifetime` duration, `endpoint_validation_policy`, `attribute_statement_policy`, `friendly_name_randomization_policy`, `nameid_format_precedence`, `request_signing_requirement`) on top of Saml/SamlAssertion/Saml2. Two ISO-8601 durations. Response `TrustRelationshipSummary`; errors `400`/`401`/`404`/`409`. DTO `Saml2EcpProfileConfigurationRequest`; mapper `TrustRelationshipMapper.updateSaml2EcpProfileConfiguration`; tests in [`trust_dto_mapper_tests.md`](./trust_dto_mapper_tests.md).
...
- [x] **Set released attributes** — `PUT /trust-relationships/{id}/released-attributes` — `updateReleasedAttributes` (forbidden from `ACTIVATING` → `409`). Full replacement; body `{ attributes: [ { id, display_name } ] }`, empty array clears. Each item needs an id and non-blank display name. Response `TrustRelationshipSummary`; errors `400`/`401`/`404`/`409`. DTO `UpdateReleasedAttributesRequest` (items reuse the bidirectional `ReleasedAttributeDto`); mapper `TrustRelationshipMapper.updateReleasedAttributes`; tests in [`trust_dto_mapper_tests.md`](./trust_dto_mapper_tests.md).
...
- [x] **Read released attributes** — `GET /trust-relationships/{id}/released-attributes` (`getTrustRelationshipReleasedAttributes`) — `ReleasedAttributesView` (`{ attributes: [ {id, display_name} ] }`, empty when none). `200`/`401`/`404`. Mapper `TrustRelationshipMapper.toReleasedAttributesView` (reuses the detail's released-attributes projection); tests in [`trust_dto_mapper_tests.md`](./trust_dto_mapper_tests.md).
