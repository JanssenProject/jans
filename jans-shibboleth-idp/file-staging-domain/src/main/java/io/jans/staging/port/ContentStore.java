 ...
 * hash (computed in one pass over the stream — the bytes are never fully buffered); {@link #move}
 * relocates a file from one location to another on claim; {@link #delete} drops a file during reaping.
 * All operations act within the shared document store, so {@code move} transfers no bytes over the wire.
 */
