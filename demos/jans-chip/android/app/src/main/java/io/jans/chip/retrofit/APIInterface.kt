...
interface APIInterface {
    ...
    @POST
    fun doDCR(@Body dcrRequest: DCRequest?, @Url url: String?): Call<DCResponse?>?

    @FormUrlEncoded
    @POST
    fun getAuthorizationChallenge(
        @Field("client_id") clientId: String?,
        @Field("username") username: String?,
        @Field("password") password: String?,
        @Field("state") state: String?,
    ...
    @GET
    fun verifyIntegrityTokenOnAppServer(@Url url: String?): Call<AppIntegrityResponse?>?

    @POST
    fun attestationOption(
        @Body request: AttestationOptionRequest?,
        @Url url: String?
    ): Call<AttestationOptionResponse?>?

    @POST
    fun attestationResult(
        @Body request: AttestationResultRequest?,
        @Url url: String?
    ): POST<Map<*, *>?>?

    @POST
    fun assertionOption(
        @Body request: AssertionOptionRequest?,
        @Url url: String?
    ): Call<AssertionOptionResponse?>?

    @POST
    fun assertionResult(
        @Body request: AssertionResultRequest?,
        @Url url: String?
    ): Call<Map<*, *>?>?*/
}