    ...
    func onLoginClick() {
        if validate() {
            ...
            networkManager.register { [weak self] client_id, _ in
                ...
                self?.networkManager.authorize(clientId: client_id) { [weak self] authorizeURL in
                    DispatchQueue.main.async {
                        self?.presenter.handle(authorizedUrl: authorizeURL)
                    }
                }
            }
        ...
        if let queryItems = components.queryItems {
            if let codeQuery = queryItems.first(where: { $0.name == "code" }), let code = codeQuery.value {
                ...
                networkManager.getToken(code: code) { [weak self] token in
                    ...
                    KeychainManager.shared.idToken = token.id_token
                    KeychainManager.shared.deviceToken = token.device_token
