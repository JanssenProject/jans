...
export default function AuthFlowInputs({
  ...
  React.useEffect(() => {
    (async () => {
      ...
      setAcrValueOption(
        client?.acrValuesSupported.map((ele) => createOption(ele))
      );
      ...
      chrome.storage.local.remove(["loginDetails"], function () {
        ...
        if (error) {
          ...
          window.location.href = `${
            client.endSessionEndpoint
          }?state=${uuidv4()}&post_logout_redirect_uri=${chrome.runtime.getURL(
            "options.html"
          )}&id_token_hint=${JSON.parse(loginDetails).loginDetails.id_token}`;
        }
      });
    });
  };
          ...
          >
            ...
            <Autocomplete
              ...
              onChange={(event, newValue, reason, details) => {
                if (reason === "removeOption") {
                  ...
                  setSelectedAcr([
                    { id: undefined, name: details.option.name },
                  ]);
            ...
            <FormControlLabel
              control={
                <Checkbox
                  color="success"
                  onChange={() => setDisplayToken(!displayToken)}
                />
              }
              label="Display Access Token and ID Token after authentication"
            />
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose}>Cancel</Button>
          <Button type="submit" onClick={triggerCodeFlow} disabled={loading}>
            Trigger Auth Flow
          </Button>
        </DialogActions>
      </Dialog>
