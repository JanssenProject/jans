...
export default function RegisterClient({ isOpen, handleDialog }) {
  ...
  const REGISTRATION_ERROR =
    "Error in registration. Check web console for logs.";
  ...
  const validateIssuer = async (e) => {
    ...
    try {
      const opConfiguration = await getOpenidConfiguration(
        openIdConfigurationURL
      );
      if (!opConfiguration || !opConfiguration.data.issuer) {
        setIssuerError(
          "Invalid input. Either enter correct Issuer or OpenID Configuration URL."
        );
        e.target.value = "";
      }
  ...
  const registerOIDCClient = async (registration_endpoint, registerObj) => {
    try {
      ...
      setErrorMessage(
        "Error in fetching Openid configuration. Check error log on console."
      );
    }
  };
  ...
  const registerClient = async () => {
    try {
      ...
      if (issuer === 0) {
        setIssuerError(
          "Issuer cannot be left blank. Either enter correct Issuer or OpenID Configuration URL."
        );
        return;
      }
      ...
      const issuerUrl =
        opConfigurationEndpointURL.protocol +
        "//" +
        opConfigurationEndpointURL.hostname;
      const scopes = selectedScopes.map((ele) => ele.name).join(" ");

      const openidConfig = await getOpenidConfiguration(
        opConfigurationEndpoint
      );

      if (openidConfig != undefined) {
        chrome.storage.local
          .set({ opConfiguration: openidConfig.data })
          .then(() => {
        ...
        if (!!expireAt) {
          registerObj.lifetime =
            (expireAt.valueOf() - moment().valueOf()) / 1000;
        }

        const registrationResp = await registerOIDCClient(
          registrationUrl,
          registerObj
        );
        ...
        if (registrationResp !== undefined) {
          ...
          console.log(
            "oidcClient is set for client_id: " +
              registrationResp.data.client_id
          );
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button type="submit" onClick={registerClient} disabled={loading}>
            Register
          </Button>
        </DialogActions>
      </Dialog>
