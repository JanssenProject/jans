...
def backupTrustStores():
    pass
    copyFile(defaultJavaTrustStore, "%s/%s" % (
        bu_folder, os.path.split(defaultJavaTrustStore)[0]
    ))
...
def genProperties():
    props = {}
    props['ldapPass'] = ldap_password
    props['hostname'] = getOutput([hostname])
    props['inumAppliance'] = getOutput([grep, "^inum",
                                        "%s/ldif/appliance.ldif" % bu_folder]
                                       ).split("\n")[0].split(":")[-1].strip()
    ...
    props['encode_salt'] = getOutput(
        [cat, "%s/opt/tomcat/conf/salt" % bu_folder]
    ).split("=")[-1].strip()
...
def prepareProperties(setupFile):
