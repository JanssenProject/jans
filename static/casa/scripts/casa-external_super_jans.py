# Based on oxAuth SuperJanssen.ExternalAuthenticator.py
...
from org.jans.oxauth.service import AuthenticationService, UserService, SessionIdService
...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def init(self, customScript, configurationAttributes):
        ...
        if configurationAttributes.containsKey("superjans_android_download_url"):
            self.androidUrl = configurationAttributes.get("superjans_android_download_url").getValue2()

        self.IOSUrl = None
        if configurationAttributes.containsKey("superjans_ios_download_url"):
            self.IOSUrl = configurationAttributes.get("superjans_ios_download_url").getValue2()
        ...
        self.use_super_jans_group = False
        if configurationAttributes.containsKey("super_jans_group"):
            self.super_jans_group = configurationAttributes.get("super_jans_group").getValue2()
            self.use_super_jans_group = True
            print "Super-Janssen.. Initialization. Using super_jans only if user belong to group: %s" % self.super_jans_group
        ...
        if self.use_super_jans_group or self.use_audit_group:
    ...
    def authenticate(self, configurationAttributes, requestParameters, step):
        ...
        if step == 1:
            ...
            if self.oneStep:
                ...
                identity.setWorkingParameter("super_jans_count_login_steps", 1)
                ...
                if (self.use_super_jans_group):
                    print "Super-Janssen.. Authenticate for step 1. Checking if user belong to super_jans group"
                    is_member_super_jans_group = self.isUserMemberOfGroup(authenticated_user, self.audit_attribute, self.super_jans_group)
                    if (is_member_super_jans_group):
                        print "Super-Janssen.. Authenticate for step 1. User '%s' member of super_jans group" % authenticated_user.getUserId()
                        super_jans_count_login_steps = 2
                    else:
                        if self.use_audit_group:
                            self.processAuditGroup(authenticated_user, self.audit_attribute, self.audit_group)
                        super_jans_count_login_steps = 1

                    identity.setWorkingParameter("super_jans_count_login_steps", super_jans_count_login_steps)

                    if super_jans_count_login_steps == 1:
                        return True
                ...
                identity.setWorkingParameter("super_jans_auth_method", auth_method)
            ...
            if self.oneStep and session_device_status['enroll']:
                ...
                super_jans_request = json.loads(session_device_status['super_jans_request'])
                auth_method = super_jans_request['method']
    ...
    def prepareForStep(self, configurationAttributes, requestParameters, step):
        ...
        if step == 1:
            ...
            if self.oneStep:
                ...
                super_jans_request_dictionary = {'app': client_redirect_uri,
                                   'issuer': issuer,
                                   'state': session_id,
                                   'licensed': self.valid_license,
                                   'created': DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(ZonedDateTime.now().withNano(0))}

                self.addGeolocationData(session_attributes, super_jans_request_dictionary)

                super_jans_request = json.dumps(super_jans_request_dictionary, separators=(',',':'))
                print "Super-Janssen.. Prepare for step 1. Prepared super_jans_request:", super_jans_request

                identity.setWorkingParameter("super_jans_request", super_jans_request)
            elif self.twoStep:
            ...
            if session_attributes.containsKey("super_jans_request"):
               super_jans_request = session_attributes.get("super_jans_request")
               if not StringHelper.equalsIgnoreCase(super_jans_request, "timeout"):
            ...
            auth_method = session_attributes.get("super_jans_auth_method")
            ...
            super_jans_request_dictionary = {'username': user.getUserId(),
            ...
            super_jans_request = json.dumps(super_jans_request_dictionary, separators=(',',':'))
            print "Super-Janssen.. Prepare for step 2. Prepared super_jans_request:", super_jans_request

            identity.setWorkingParameter("super_jans_request", super_jans_request)
            identity.setWorkingParameter("super_jans_auth_method", auth_method)

            if auth_method in ['authenticate']:
                self.sendPushNotification(client_redirect_uri, user, super_jans_request)
    ...
    def getNextStep(self, configurationAttributes, requestParameters, step):
        ...
        if retry_current_step:
            ...
            # Remove old QR code
            identity.setWorkingParameter("super_jans_request", "timeout")
    ...
    def getExtraParametersForStep(self, configurationAttributes, step):
        ...
        #Parameter "display_register_action" used in default login.xhtml page of Janssen. Server will not be set
        if step == 1:
            if self.oneStep:
                return Arrays.asList("super_jans_request")
            elif self.twoStep:
                return Arrays.asList("display_register_action")
        elif step == 2:
            return Arrays.asList("super_jans_auth_method", "super_jans_request")

        return None

    def getCountAuthenticationSteps(self, configurationAttributes):
        identity = CdiUtil.bean(Identity)
        if identity.isSetWorkingParameter("super_jans_count_login_steps"):
            return identity.getWorkingParameter("super_jans_count_login_steps")
        else:
            return 2
    ...
    def getPageForStep(self, configurationAttributes, step):
        if step == 1:
            ...
            if self.oneStep:
                ...
                identity = CdiUtil.bean(Identity)
                authmethod = identity.getWorkingParameter("super_jans_auth_method")
    ...
    def getSessionDeviceStatus(self, session_attributes, user_name):
        ...
        if not session_attributes.containsKey("super_jans_request"):
        ...
        session_device_status = {"super_jans_request": super_jans_request, "device_id": u2f_device_id, "user_inum" : user_inum, "enroll" : enroll, "one_step" : one_step}
    ...
    def initPushNotificationService(self, configurationAttributes):
        ...
        if configurationAttributes.containsKey("notification_service_mode"):
            ...
            if StringHelper.equalsIgnoreCase(notificationServiceMode, "sns"):
                ...
                return self.initJanssen.PushNotificationService(configurationAttributes)
    ...
    def initJanssen.PushNotificationService(self, configurationAttributes):
        print "Super-Janssen.. Initialize Janssen. notification services"

        self.pushJanssen.Mode = True
        ...
        try:
            metadataConfiguration = notifyClientFactory.createMetaDataConfigurationService(jans_server_uri).getMetadataConfiguration()
        except:
        ...
        if android_creds["enabled"]:
            ...
            try:
                ...
                # Ignore exception. Password is not encrypted
                print "Super-Janssen.. Initialize Janssen. notification services. Assuming that 'jans_secret_access_key' in not encrypted"
    ...
    def loadPushNotificationCreds(self, configurationAttributes):
        ...
        super_jans_creds_file = configurationAttributes.get("credentials_file").getValue2()

        # Load credentials from file
        f = open(super_jans_creds_file, 'r')
        try:
            creds = json.loads(f.read())
        except:
            print "Super-Janssen.. Initialize notification services. Failed to load credentials from file:", super_jans_creds_file
            return None
        finally:
            f.close()
    ...
    def sendPushNotification(self, client_redirect_uri, user, super_jans_request):
        try:
            self.sendPushNotificationImpl(client_redirect_uri, user, super_jans_request)
        except:
            print "Super-Janssen.. Send push notification. Failed to send push notification: ", sys.exc_info()[1]

    def sendPushNotificationImpl(self, client_redirect_uri, user, super_jans_request):
            ...
            for u2f_device in u2f_devices_list:
                ...
                if StringHelper.equalsIgnoreCase(platform, "ios") and StringHelper.isNotEmpty(push_token):
                    ...
                    if self.pushAppleService == None:
                        ...
                        if self.pushSnsMode or self.pushJanssen.Mode:
                            pushSnsService = CdiUtil.bean(PushSnsService)
                            ...
                            additional_fields = { "request" : super_jans_request }
                ...
                if StringHelper.equalsIgnoreCase(platform, "android") and StringHelper.isNotEmpty(push_token):
                    ...
                    if self.pushAndroidService == None:
                        ...
                        if self.pushSnsMode or self.pushJanssen.Mode:
                            ...
                            msgBuilder = Message.Builder().addData("message", super_jans_request).addData("title", title).collapseKey("single").contentAvailable(True)
                            push_message = msgBuilder.build()
    ...
    def setRequestScopedParameters(self, identity, step):
        ...
        if self.customLabel != None:
            identity.setWorkingParameter("super_jans_label", self.customLabel)

        identity.setWorkingParameter("download_url",downloadMap)
        identity.setWorkingParameter("super_jans_qr_options", self.customQrOptions)

    def addGeolocationData(self, session_attributes, super_jans_request_dictionary):
        if session_attributes.containsKey("remote_ip"):
            ...
            if StringHelper.isNotEmpty(remote_ip):
                print "Super-Janssen.. Prepare for step 2. Adding req_ip and req_loc to super_jans_request"
                super_jans_request_dictionary['req_ip'] = remote_ip
                ...
                super_jans_request_dictionary['req_loc'] = remote_loc_encoded
