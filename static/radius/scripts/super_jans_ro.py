# Super Janssen Radius Resource Owner Password Credentials Script
...
from org.jans.oxauth.model.config import ConfigurationFactory , Constants
...
from jans_common import PushNotificationManager, NetworkApi, GeolocationData, SuperJansRequestBuilder
...
class ResourceOwnerPasswordCredentials(ResourceOwnerPasswordCredentialsType):
    ...
    def init(self, customScript, configurationAttributes):
        ...
        print "Super-Janssen-RO Init"
        if not configurationAttributes.containsKey("application_id"):
            print "Super-Janssen-Radius RO PW Init Failed. application_id property is required"
            return False
        
        if not configurationAttributes.containsKey("credentials_file"):
            print "Super-Janssen-RO Init Failed. credentials_file is required"
            return False
    ...
    def destroy(self, configurationAttributes):
        ...
        print "Super-Janssen-RO. Destroy"
        self.pushNotificationManager = None
        print "Super-Janssen-RO. Destroyed Successfully"
    ...
    def authenticate(self, context): 
        if self.perform_preliminary_user_authentication(context) == False:
            print "Super-Janssen-RO. User authentication state not validated"
            return False
        ...
        if StringHelper.equalsIgnoreCase(step,self.initiateAuthStepName):
            ...
            print "Super-Janssen-RO. Unknown authentication step '%s'" % step
            return False
    
    def initiate_authentication(self, context):
        print "Super-Janssen-RO initiatate_authentication"
        ...
        if StringHelper.equalsIgnoreCase(authscheme,self.oneStepAuthScheme):
            print "Super-Janssen-RO using one-step authentication"
            ...
            print "Super-Janssen-RO using two-step authentication"
            ...
            if not self.send_push_notification_to_user(sessionId,context):
                ...
                context.setUser(None)
                return False
            print "Super-Janssen-RO initiate_authentication complete"
            return True
        else:
            print "Super-Janssen-RO. Unknown authentication scheme specified '%s'" % authscheme
            context.setUser(None)
            return False
    ...
    def resend_push_notification(self,context):
        print "Super-Janssen-RO resend_push_notification"

        sessionIdService = CdiUtil.bean(SessionIdService)
        session_id = context.getHttpRequest().getParameter(self.sessionIdParamName)
        if session_id == None:
            print "Super-Janssen-RO. No session_id was specified for resend_push_notification"
            context.setUser(None)
            return False
        
        sessionId = sessionIdService.getSessionId(session_id)
        if sessionId == None:
            print "Super-Janssen-RO. Session '%s' does not exist or has expired" % session_id
            context.setUser(None)
            return False
        
        client = CdiUtil.bean(Identity).getSessionClient().getClient()
        if not self.verify_session_ownership(sessionId,context.getUser(),client):
            print "Super-Janssen-RO. resend_push_notification_failed due to invalid session ownership"
            context.setUser(None)
            return False
        
        self.send_push_notification_to_user(sessionId,context)
        print "Super-Janssen-RO resend_push_notification complete"
        return True
    
    def verify_authentication(self, context):
        print "Super-Janssen-RO verify_authentication"
        session_id = context.getHttpRequest().getParameter(self.sessionIdParamName)
        sessionId = CdiUtil.bean(SessionIdService).getSessionId(session_id)
        if sessionId == None:
            print "Super-Janssen-RO.verify_authentication failed. Session {%s} does not exist or has expired" % session_id
            context.setUser(None)
            return False
        
        client = CdiUtil.bean(Identity).getSessionClient().getClient()
        if not self.verify_session_ownership(sessionId,context.getUser(),client):
            print "Super-Janssen-RO. verify_authentication failed due to invalid session ownership"
            context.setUser(None)
            return False
        
        if not self.is_session_authenticated(sessionId):
            print "Super-Janssen-Ro. verify_authentication failed. Session is not authenticated"
            context.setUser(None)
            return False
        
        print "Super-Janssen-RO verify_authentication complete"
        return True
    ...
    def perform_preliminary_user_authentication(self, context):
        ...
        if self.authWithoutPassword:
            ...
            if user == None:
                print "Super-Janssen-RO. User '%s' not found" % username
                return False
            context.setUser(user)
            print "Super-Janssen-RO. User '%s' authenticated without password" % username
            return True
        ...
        if authService.authenticate(username, password) == False:
            print "Super-Janssen-RO. Could not authenticate user '%s' " % username
            return False
    ...
    def new_unauthenticated_session(self,user,client):
        ...
        print "Super-Janssen-RO. Generated session id. DN: '%s'" % sessionId.getDn()
        return sessionId
    ...
    def send_push_notification_to_user(self, sessionId,context):
        ...
        srbuilder = SuperJansRequestBuilder()
        ...
        if device_count == 0:
            print "User %s has no device enrolled for Super-Janssen authentication" % user.getUserId()
            return False
        return True
    ...
    def is_session_authenticated(self, sessionId):
        ...
        if state == None:
            print "Super-Janssen-RO. Session {%s} has no state variable set" % sessionId.getId()
            return False
        ...
        if state_unauthenticated and (custom_state_declined or custom_state_expired):
            print "Super-Janssen-RO. Session {%s} isn't authenticated" % sessionId.getId()
            return False
        
        if state_authenticated or (state_unauthenticated and custom_stated_approved):
            print "Super-Janssen-RO. Session {%s} is authenticated" % sessionId.getId()
            return True
    ...
    def verify_session_ownership(self, sessionId, user, client):
        ...
        if not StringHelper.equalsIgnoreCase(client.getClientId(),client_id):
            print "Super-Janssen-RO. Session {%s} client_id mismatch" % sessionId.getId()
            return False
        
        user_id = session_attributes.get(Constants.AUTHENTICATED_USER)
        if not StringHelper.equalsIgnoreCase(user_id,user.getUserId()):
            print "Super-Janssen-RO. Session {%s} user_id mismatch" % sessionId.getId() 
            return False
