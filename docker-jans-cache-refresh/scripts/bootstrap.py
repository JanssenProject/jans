...
def configure_logging():
    # default config
    config = {
        "cacherefresh_log_target": "STDOUT",
        "cacherefresh_log_level": "INFO",
    ...
    try:
        custom_config = json.loads(os.environ.get("CN_CACHE_REFRESH_APP_LOGGERS", "{}"))
    ...
    if not isinstance(custom_config, dict):
        logger.warning("Invalid data type for CN_CACHE_REFRESH_APP_LOGGERS; fallback to defaults")
        custom_config = {}
    ...
    logfile = "/opt/jans/jetty/jans-cache-refresh/resources/log4j2.xml"
    tmpl = Template(txt)
...
class PersistenceSetup:
    ...
    def ldif_files(self) -> list[str]:
        files = [
            f"/app/templates/jans-cache-refresh/{file_}"
