...
def test_config_manager(adapter, adapter_cls):
    ...
    os.environ["JANS_CONFIG_ADAPTER"] = adapter
    manager = ConfigManager()

    assert manager.adapter.__class__.__name__ == adapter_cls
    os.environ.pop("JANS_CONFIG_ADAPTER", None)
...
def test_secret_manager(adapter, adapter_cls):
    ...
    os.environ["JANS_SECRET_ADAPTER"] = adapter
    manager = SecretManager()

    assert manager.adapter.__class__.__name__ == adapter_cls
    os.environ.pop("JANS_SECRET_ADAPTER", None)
