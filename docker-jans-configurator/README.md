...
For bleeding-edge/unstable version, use `janssenproject/configurator:1.0.14_dev`.
...
1. Mount the `generate.json` into container:
    ...
    docker run \
        ...
        janssenproject/configurator:1.0.14_dev load
    ```
        ...
        spec:
          ...
          containers:
            - name: configurator-load
              image: janssenproject/configurator:1.0.14_dev
              volumeMounts:
...
docker run \
    ...
    -v /path/to/vault_role_id.txt:/etc/certs/vault_role_id \
    -v /path/to/vault_secret_id.txt:/etc/certs/vault_secret_id \
    janssenproject/configurator:1.0.14_dev dump
```
