...
For bleeding-edge/unstable version, use `janssenproject/configurator:1.0.10_dev`.
...
1. Mount the `generate.json` into container:
    ...
    docker run \
        ...
        janssenproject/configurator:1.0.10_dev load
    ```
...
docker run \
    ...
    -v /path/to/vault_role_id.txt:/etc/certs/vault_role_id \
    -v /path/to/vault_secret_id.txt:/etc/certs/vault_secret_id \
    janssenproject/configurator:1.0.10_dev dump
```
