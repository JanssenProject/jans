...
pub struct PolicyStore {
	pub(crate) inner: cedarling::PolicyStore,
}

fn get_store_from_config(config: cedarling::PolicyStoreConfig) -> PyResult<PolicyStore> {
	let store = config
		.get_policy()
...
impl PolicyStore {
	...
	fn new(input: Bound<'_, PyAny>) -> PyResult<Self> {
		let store: cedarling::PolicyStore = from_pyobject(input)?;
		Ok(PolicyStore { inner: store })
	}
	...
	pub fn from_raw_json(raw_json: &Bound<'_, PyString>) -> PyResult<Self> {
		...
		get_store_from_config(cedarling::PolicyStoreConfig::JsonRaw(rust_str))
	}

	#[staticmethod]
	pub fn from_filepath(filepath: &Bound<'_, PyString>) -> PyResult<Self> {
		let rust_str = to_rust_string(filepath)?;
		get_store_from_config(cedarling::PolicyStoreConfig::File(rust_str.into()))
	}

	#[staticmethod]
	pub fn from_remote_uri(uri: &Bound<'_, PyString>) -> PyResult<Self> {
		let rust_str = to_rust_string(uri)?;
		get_store_from_config(cedarling::PolicyStoreConfig::RemoteURI(rust_str.into()))
	}
}
