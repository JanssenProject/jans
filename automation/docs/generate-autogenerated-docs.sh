...
# Destination directories for the generated docs. Verify they exist before the
# (slow) Maven compiles so a docs-tree rename (as happened when docs/admin became
# docs/janssen-server) fails immediately instead of after a full build, and never
# silently leaves the published docs stale.
