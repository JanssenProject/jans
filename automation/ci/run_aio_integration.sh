...
AIO_IMAGE_TAG="ghcr.io/janssenproject/jans/all-in-one:2.4.1-1"
...
# excluded so a stale 2.4.1 from another commit can't be resolved silently; the cedarling
...
if [ -z "${AIO_IMAGE:-}" ]; then
  ...
  find "$HOME/.m2/repository/io/jans" -type f -path "*/2.4.1/*" \
