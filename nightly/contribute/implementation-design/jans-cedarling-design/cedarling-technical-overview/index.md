...
1. [Bootstrap Properties](https://docs.jans.io/nightly/cedarling/reference/cedarling-properties/index.md)
1. [Policy Store](https://docs.jans.io/nightly/cedarling/reference/cedarling-policy-store/index.md)
...
1. Validate JWT via `JwtService`
1. Build entities using `EntityBuilder`.
1. Evaluate entities against policies in `authz`.
1. Return `Ok(AuthorizeResult)` or `Err(AuthorizeError)`
...
Criteria for valid JWT: * Comes from a trusted issuer: * Has a defined [token metadata](https://docs.jans.io/nightly/cedarling/reference/cedarling-policy-store/#token-metadata-schema)
...
Mappings are configured via the Token Entity Metadata Schema's `claim mapping` field in the [Policy Store](https://docs.jans.io/nightly/cedarling/reference/cedarling-policy-store/index.md).
...
Download the latest builds from: [Dynamic Download](https://docs.jans.io/nightly/janssen-server/install/vm-install/dynamic-download/index.md).
