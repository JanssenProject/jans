...
- `cedarling.opa.authorize_multi_issuer(input)`: Calls the [multi-issuer authorization](https://docs.jans.io/nightly/cedarling/reference/cedarling-authz/#multi-issuer-authorization-authorize_multi_issuer-recommended) interface.
...
- `cedarling.opa.authorize_unsigned(input)`: Calls the [unsigned authorization](https://docs.jans.io/nightly/cedarling/reference/cedarling-authz/#unsigned-authorization-authorize_unsigned) interface.
...
1. Create or edit the plugin configuration file `demo/opa-config.json`
   ...
   - `bootstrap_config`: Bootstrap configuration dictionary for the Cedarling instance. Refer to the documentation for [bootstrap](https://docs.jans.io/nightly/cedarling/reference/cedarling-properties/index.md) and [policy store](https://docs.jans.io/nightly/cedarling/reference/cedarling-policy-store/index.md) configuration.
...
- [Gating Terraform with Cedarling-OPA](https://docs.jans.io/nightly/cedarling/integrations/terraform-authz/index.md) — role-based control over `terraform plan / apply / destroy` across dev, staging, and production workspaces, enforced by a shell wrapper before any cloud change is made.
- [Terraform Authorization with GitHub Actions OIDC JWTs](https://docs.jans.io/nightly/cedarling/integrations/terraform-authz-jwt/index.md) — CI/CD-first variant using `authorize_multi_issuer`; Cedarling validates GitHub OIDC tokens and evaluates Cedar policies that check JWT claims (`repository`, `ref`, `environment`) to gate Terraform operations with no service-account secrets.
