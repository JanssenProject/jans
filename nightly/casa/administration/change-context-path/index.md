...
1. Edit tag `<Set name="contextPath">` in file `/opt/jans/jetty/jans-casa/webapps/jans-casa.xml` with the new path you want to use. For example, if you chose `/creds`, you would do the following:

   ```
     <Set name="contextPath">/creds</Set>
   ```

1. Edit tag `<Set name="contextPath">` in file `/opt/jans/jetty/jans-casa/webapps/jans-casa_web_resources.xml` appropriately with the new path you want to use.
...
1. Wait for around 1 minute (so the server picks the script changes), then restart Casa and Apache services. Use this [page](https://docs.jans.io/nightly/janssen-server/vm-ops/restarting-services/index.md) as a guide
