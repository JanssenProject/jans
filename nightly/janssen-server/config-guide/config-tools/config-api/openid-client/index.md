...
`GET` `/` `(gets list of OpenID Connect clients based on search parameters)`
...
`GET` `/{inum}` `(gets OpenID Connect client based on inum)`
...
`POST` `/{inum}` `(creates a new OpenID Connect client)`
...
`PUT` `/{inum}` `(updates an existing OpenID Connect client)`
...
`PATCH` `/{inum}` `(patches an existing OpenID Connect client)`
...
> | --------- | ------------------- | --------------------------------------------------------- |
> | `200`     | `application/json ` | `OpenID Connect client details`                           |
> | `404`     | `application/json`  | `{"code":"404","message":"The requested  doesn't exist"}` |
> | `401`     | `application/json`  | `{"code":"401","message":"Unauthorized"}`                 |
> | `500`     | `application/json`  | `{"code":"500","message":"Error msg"}`                    |
...
`DELETE` `/{inum}` `(deletes an existing OpenID Connect client)`
