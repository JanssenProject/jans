...
> Prerequisite: Know how to use the Janssen CLI in [command-line mode](https://docs.jans.io/nightly/janssen-server/config-guide/config-tools/jans-cli/index.md)
...
It returns all the custom scripts that are related to the `CLIENT_REGISTRATION` type available in the Janssen Server. You will find the name of all the types [here](https://docs.jans.io/nightly/janssen-server/config-guide/custom-scripts-config/#name-of-the-type-of-scripts)
