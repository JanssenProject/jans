...
User sessions in database can be searched, viewed and deleted using TUI, command line an rest-api

## Use Command-line

In the Janssen Server, you can search, delete, view sessions under `ConfigurationLogging` task using the command below:
...
As an example search sessions of user **admin** expiration date greater than October 12, 2024
...
To start searching and deleting sessions, navigate to `Auth Server`->`Sessions`. This brings up a screen as shown below with searching and deleting sessions.
...
To use rest-api you need an access token. You can get access token by executing the following command if you previously authorized by TUI:
...
As an example let us search sessions of user **admin** expiration date greater than October 12, 2024
...
Navigate to **Auth - Session Management** for other endpoints of managing sessions in config-api [swagger file](https://raw.githubusercontent.com/JanssenProject/jans/nightly/jans-config-api/docs/jans-config-api-swagger.yaml).
