...
Currently, the Janssen server supports an OAuth-based mechanism to protect these endpoints; you can find more information in this regard [here](https://docs.jans.io/nightly/janssen-server/scim/oauth-protection/index.md). Nonetheless, it is recommended not to expose this API outside the organization's network. Ideally administrators should block any external attempt to access URLs under the path `https://your-jans-server/jans-scim`.
