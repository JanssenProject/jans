...
Please visit this [page](https://docs.jans.io/nightly/janssen-server/config-guide/auth-server-config/attribute-configuration/index.md) for a more detailed explanation. When registering the attribute in the **TUI**, please ensure you have set the `Include in SCIM Extension` parameter to `true`.
...
To customize the URI associated to the extension (whose default value is `urn:ietf:params:scim:schemas:extension:gluu:2.0:User` ), you can use TUI:
