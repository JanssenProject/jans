...
| config.configmap.cnGoogleSecretManagerServiceAccount | string | `"SWFtTm90YVNlcnZpY2VBY2NvdW50Q2hhbmdlTWV0b09uZQo="` | Document store type to use for shibboleth files JCA or LOCAL. Note that if JCA is selected Apache Jackrabbit will be used. Jackrabbit also enables loading custom files across all services easily. |
...
| config.configmap.cnPersistenceLdapMapping | string | `"default"` | Boolean flag to enable/disable passport chart |
...
| config.configmap.cnSecretKubernetesSecret | string | `"cn"` | Enable SAML-related features; UI menu, etc. |
...
| global.cnPersistenceType | string | `"sql"` | Boolean flag if enabled will enable jackrabbit in cluster mode with Postgres. |
