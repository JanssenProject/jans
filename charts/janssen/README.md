...
| gateway-api.additionalConfig.nginx | object | `{"enableGrpcRewriteSnippets":false}` | Configuration for NGINX Fabric. |
| gateway-api.additionalConfig.nginx.enableGrpcRewriteSnippets | bool | `false` | Enable URL rewrite to proxy gRPC requests to /jans-auth. Snippet support must be enabled during NGINX installation (otherwise endpoints will return HTTP status code 500). See https://docs.nginx.com/nginx-gateway-fabric/traffic-management/snippets#setup |
...
| shibboleth-idp.image.tag | string | `"5.1.6_dev"` | Image tag to use for deploying. |
