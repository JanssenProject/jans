...
![Version: 1.0.22-dev](https://img.shields.io/badge/Version-1.0.22--dev-informational?style=flat-square) ![AppVersion: 1.0.22-dev](https://img.shields.io/badge/AppVersion-1.0.22--dev-informational?style=flat-square)
...
|------------|------|---------|
|  | auth-server | 1.0.22-dev |
|  | auth-server-key-rotation | 1.0.22-dev |
|  | casa | 1.0.22-dev |
|  | config | 1.0.22-dev |
|  | config-api | 1.0.22-dev |
|  | fido2 | 1.0.22-dev |
|  | link | 1.0.22-dev |
|  | nginx-ingress | 1.0.22-dev |
|  | opendj | 1.0.22-dev |
|  | persistence | 1.0.22-dev |
|  | saml | 1.0.22-dev |
|  | scim | 1.0.22-dev |
...
| config.configmap.kcDbSchema | string | `"keycloak"` | Database schema name (note that PostgreSQL may using "public" schema). |
| config.configmap.kcDbUrlDatabase | string | `"keycloak"` | Keycloak database name |
| config.configmap.kcDbUrlHost | string | `"mysql.kc.svc.cluster.local"` | Database host |
| config.configmap.kcDbUrlPort | int | `3306` | Database port |
| config.configmap.kcDbUrlProperties | string | `"?useUnicode=true&characterEncoding=UTF-8&character_set_server=utf8mb4"` | Keycloak database connection properties |
...
| config.configmap.quarkusTransactionEnableRecovery | bool | `false` | Quarkus transaction recovery |
...
| saml | object | `{"additionalAnnotations":{},"additionalLabels":{},"customScripts":[],"dnsConfig":{},"dnsPolicy":"","hpa":{"behavior":{},"enabled":true,"maxReplicas":10,"metrics":[],"minReplicas":1,"targetCPUUtilizationPercentage":50},"image":{"pullPolicy":"IfNotPresent","pullSecrets":[],"repository":"ghcr.io/janssenproject/jans/saml","tag":"1.0.22_dev"},"lifecycle":{},"livenessProbe":{"exec":{"command":["python3","/app/scripts/healthcheck.py"]},"failureThreshold":5,"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":5},"pdb":{"enabled":true,"maxUnavailable":"90%"},"readinessProbe":{"exec":{"command":["python3","/app/scripts/healthcheck.py"]},"failureThreshold":5,"initialDelaySeconds":25,"periodSeconds":25,"timeoutSeconds":5},"replicas":1,"resources":{"limits":{"cpu":"500m","memory":"1000Mi"},"requests":{"cpu":"500m","memory":"1000Mi"}},"topologySpreadConstraints":{},"usrEnvs":{"normal":{},"secret":{}},"volumeMounts":[],"volumes":[]}` | SAML. |
...
| saml.livenessProbe | object | `{"exec":{"command":["python3","/app/scripts/healthcheck.py"]},"failureThreshold":5,"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":5}` | Configure the liveness healthcheck for the auth server if needed. |
