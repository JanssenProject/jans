...
![Version: 1.0.15-dev](https://img.shields.io/badge/Version-1.0.15--dev-informational?style=flat-square) ![AppVersion: 1.0.15-dev](https://img.shields.io/badge/AppVersion-1.0.15--dev-informational?style=flat-square)
...
|------------|------|---------|
|  | auth-server | 1.0.15-dev |
|  | auth-server-key-rotation | 1.0.15-dev |
|  | cache-refresh | 1.0.15-dev |
|  | config | 1.0.15-dev |
|  | config-api | 1.0.15-dev |
|  | fido2 | 1.0.15-dev |
|  | nginx-ingress | 1.0.15-dev |
|  | opendj | 1.0.15-dev |
|  | persistence | 1.0.15-dev |
|  | scim | 1.0.15-dev |
...
| cache-refresh | object | `{"additionalAnnotations":{},"additionalLabels":{},"dnsConfig":{},"dnsPolicy":"","hpa":{"behavior":{},"enabled":true,"maxReplicas":10,"metrics":[],"minReplicas":1,"targetCPUUtilizationPercentage":50},"image":{"pullPolicy":"IfNotPresent","pullSecrets":[],"repository":"ghcr.io/janssenproject/jans/cache-refresh","tag":"1.0.15_dev"},"lifecycle":{},"livenessProbe":{"httpGet":{"path":"/jans-cache-refresh/api/v1/health/live","port":8074},"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":5},"pdb":{"enabled":true,"maxUnavailable":"90%"},"readinessProbe":{"httpGet":{"path":"jans-cache-refresh/api/v1/health/ready","port":8074},"initialDelaySeconds":25,"periodSeconds":25,"timeoutSeconds":5},"replicas":1,"resources":{"limits":{"cpu":"500m","memory":"1000Mi"},"requests":{"cpu":"500m","memory":"1000Mi"}},"topologySpreadConstraints":{},"usrEnvs":{"normal":{},"secret":{}},"volumeMounts":[],"volumes":[]}` | Cache Refresh. |
| cache-refresh.additionalAnnotations | object | `{}` | Additional annotations that will be added across the gateway in the format of {cert-manager.io/issuer: "letsencrypt-prod"} |
| cache-refresh.additionalLabels | object | `{}` | Additional labels that will be added across the gateway in the format of {mylabel: "myapp"} |
| cache-refresh.dnsConfig | object | `{}` | Add custom dns config |
| cache-refresh.dnsPolicy | string | `""` | Add custom dns policy |
| cache-refresh.hpa | object | `{"behavior":{},"enabled":true,"maxReplicas":10,"metrics":[],"minReplicas":1,"targetCPUUtilizationPercentage":50}` | Configure the HorizontalPodAutoscaler |
| cache-refresh.hpa.behavior | object | `{}` | Scaling Policies |
| cache-refresh.hpa.metrics | list | `[]` | metrics if targetCPUUtilizationPercentage is not set |
| cache-refresh.image.pullPolicy | string | `"IfNotPresent"` | Image pullPolicy to use for deploying. |
| cache-refresh.image.pullSecrets | list | `[]` | Image Pull Secrets |
| cache-refresh.image.repository | string | `"ghcr.io/janssenproject/jans/cache-refresh"` | Image  to use for deploying. |
| cache-refresh.image.tag | string | `"1.0.15_dev"` | Image  tag to use for deploying. |
| cache-refresh.livenessProbe | object | `{"httpGet":{"path":"/jans-cache-refresh/api/v1/health/live","port":8074},"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":5}` | Configure the liveness healthcheck for the auth server if needed. |
| cache-refresh.livenessProbe.httpGet | object | `{"path":"/jans-cache-refresh/api/v1/health/live","port":8074}` | http liveness probe endpoint |
| cache-refresh.pdb | object | `{"enabled":true,"maxUnavailable":"90%"}` | Configure the PodDisruptionBudget |
| cache-refresh.readinessProbe.httpGet | object | `{"path":"jans-cache-refresh/api/v1/health/ready","port":8074}` | http readiness probe endpoint |
| cache-refresh.replicas | int | `1` | Service replica number. |
| cache-refresh.resources | object | `{"limits":{"cpu":"500m","memory":"1000Mi"},"requests":{"cpu":"500m","memory":"1000Mi"}}` | Resource specs. |
| cache-refresh.resources.limits.cpu | string | `"500m"` | CPU limit. |
| cache-refresh.resources.limits.memory | string | `"1000Mi"` | Memory limit. |
| cache-refresh.resources.requests.cpu | string | `"500m"` | CPU request. |
| cache-refresh.resources.requests.memory | string | `"1000Mi"` | Memory request. |
| cache-refresh.topologySpreadConstraints | object | `{}` | Configure the topology spread constraints. Notice this is a map NOT a list as in the upstream API https://kubernetes.io/docs/concepts/scheduling-eviction/topology-spread-constraints/ |
| cache-refresh.usrEnvs | object | `{"normal":{},"secret":{}}` | Add custom normal and secret envs to the service |
| cache-refresh.usrEnvs.normal | object | `{}` | Add custom normal envs to the service variable1: value1 |
| cache-refresh.usrEnvs.secret | object | `{}` | Add custom secret envs to the service variable1: value1 |
| cache-refresh.volumeMounts | list | `[]` | Configure any additional volumesMounts that need to be attached to the containers |
| cache-refresh.volumes | list | `[]` | Configure any additional volumes that need to be attached to the pod |
| config | object | `{"additionalAnnotations":{},"additionalLabels":{},"adminPassword":"Test1234#","city":"Austin","configmap":{"cnAwsAccessKeyId":"","cnAwsDefaultRegion":"us-west-1","cnAwsProfile":"janssen","cnAwsSecretAccessKey":"","cnAwsSecretsEndpointUrl":"","cnAwsSecretsNamePrefix":"janssen","cnAwsSecretsReplicaRegions":[],"cnCacheType":"NATIVE_PERSISTENCE","cnConfigKubernetesConfigMap":"cn","cnCouchbaseBucketPrefix":"jans","cnCouchbaseCrt":"SWFtTm90YVNlcnZpY2VBY2NvdW50Q2hhbmdlTWV0b09uZQo=","cnCouchbaseIndexNumReplica":0,"cnCouchbasePassword":"P@ssw0rd","cnCouchbaseSuperUser":"admin","cnCouchbaseSuperUserPassword":"Test1234#","cnCouchbaseUrl":"cbjanssen.default.svc.cluster.local","cnCouchbaseUser":"janssen","cnGoogleProjectId":"google-project-to-save-config-and-secrets-to","cnGoogleSecretManagerServiceAccount":"SWFtTm90YVNlcnZpY2VBY2NvdW50Q2hhbmdlTWV0b09uZQo=","cnGoogleSecretNamePrefix":"janssen","cnGoogleSecretVersionId":"latest","cnGoogleSpannerDatabaseId":"","cnGoogleSpannerInstanceId":"","cnJettyRequestHeaderSize":8192,"cnLdapUrl":"opendj:1636","cnMaxRamPercent":"75.0","cnPersistenceHybridMapping":"{}","cnRedisSentinelGroup":"","cnRedisSslTruststore":"","cnRedisType":"STANDALONE","cnRedisUrl":"redis.redis.svc.cluster.local:6379","cnRedisUseSsl":false,"cnScimProtectionMode":"OAUTH","cnSecretKubernetesSecret":"cn","cnSqlDbDialect":"mysql","cnSqlDbHost":"my-release-mysql.default.svc.cluster.local","cnSqlDbName":"jans","cnSqlDbPort":3306,"cnSqlDbSchema":"","cnSqlDbTimezone":"UTC","cnSqlDbUser":"jans","cnSqldbUserPassword":"Test1234#","lbAddr":""},"countryCode":"US","dnsConfig":{},"dnsPolicy":"","email":"support@jans.io","image":{"pullSecrets":[],"repository":"ghcr.io/janssenproject/jans/configurator","tag":"1.0.15_dev"},"ldapPassword":"P@ssw0rds","lifecycle":{},"orgName":"Janssen","redisPassword":"P@assw0rd","resources":{"limits":{"cpu":"300m","memory":"300Mi"},"requests":{"cpu":"300m","memory":"300Mi"}},"state":"TX","usrEnvs":{"normal":{},"secret":{}},"volumeMounts":[],"volumes":[]}` | Configuration parameters for setup and initial configuration secret and config layers used by Janssen services. |
...
| global.cache-refresh.appLoggers | object | `{"cacheRefreshLogLevel":"INFO","cacheRefreshLogTarget":"STDOUT","enableStdoutLogPrefix":"true","ldapStatsLogLevel":"INFO","ldapStatsLogTarget":"FILE","persistenceDurationLogLevel":"INFO","persistenceDurationLogTarget":"FILE","persistenceLogLevel":"INFO","persistenceLogTarget":"FILE","scriptLogLevel":"INFO","scriptLogTarget":"FILE"}` | App loggers can be configured to define where the logs will be redirected to and the level of each in which it should be displayed. |
| global.cache-refresh.appLoggers.cacheRefreshLogLevel | string | `"INFO"` | cacherefresh.log level |
| global.cache-refresh.appLoggers.cacheRefreshLogTarget | string | `"STDOUT"` | cacherefresh.log target |
| global.cache-refresh.appLoggers.enableStdoutLogPrefix | string | `"true"` | Enable log prefixing which enables prepending the STDOUT logs with the file name. i.e cache-refresh-persistence ===> 2022-12-20 17:49:55,744 INFO |
| global.cache-refresh.appLoggers.ldapStatsLogLevel | string | `"INFO"` | cacherefresh_persistence_ldap_statistics.log level |
| global.cache-refresh.appLoggers.ldapStatsLogTarget | string | `"FILE"` | cacherefresh_persistence_ldap_statistics.log target |
| global.cache-refresh.appLoggers.persistenceDurationLogLevel | string | `"INFO"` | cacherefresh_persistence_duration.log level |
| global.cache-refresh.appLoggers.persistenceDurationLogTarget | string | `"FILE"` | cacherefresh_persistence_duration.log target |
| global.cache-refresh.appLoggers.persistenceLogLevel | string | `"INFO"` | cacherefresh_persistence.log level |
| global.cache-refresh.appLoggers.persistenceLogTarget | string | `"FILE"` | cacherefresh_persistence.log target |
| global.cache-refresh.appLoggers.scriptLogLevel | string | `"INFO"` | cacherefresh_script.log level |
| global.cache-refresh.appLoggers.scriptLogTarget | string | `"FILE"` | cacherefresh_script.log target |
| global.cache-refresh.cacheRefreshServiceName | string | `"cache-refresh"` | Name of the cache-refresh service. Please keep it as default. |
| global.cache-refresh.enabled | bool | `false` | Boolean flag to enable/disable the cache-refresh chart. |
| global.cache-refresh.ingress | object | `{"cacheRefreshEnabled":true}` | Enable endpoints in either istio or nginx ingress depending on users choice |
