...
from flex_aio.jans_config_api.plugins import AdminUiPlugin
from flex_aio.jans_config_api.plugins import discover_plugins
from flex_aio.jans_config_api.utils import get_config_api_scope_mapping
...
logger = logging.getLogger("jans_config_api")
...
def configure_logging():
    ...
    src, dst = get_template_paths("/opt/jans/jetty/jans-config-api/resources/log4j2.xml")
    with open(src) as f:
        txt = f.read()
...
def configure_admin_ui_logging():
    ...
    src, dst = get_template_paths("/opt/jans/jetty/jans-config-api/custom/config/log4j2-adminui.xml")
    with open(src) as f:
        txt = f.read()
