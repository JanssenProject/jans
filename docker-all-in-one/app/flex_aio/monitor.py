...
logger = logging.getLogger("flex_aio")


def watch_jans_auth():
    logger.info("CHECKING jans-auth")

    try:
        req = requests.get("http://127.0.0.1:8081/jans-auth/sys/health-check")
        ...
        if req.ok:
            ...
            logger.info(f"Checking network")
