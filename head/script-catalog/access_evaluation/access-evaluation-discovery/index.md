...
The AuthZEN discovery endpoint publishes PDP (Policy Decision Point) metadata per the AuthZEN specification, including all available endpoints and capabilities.
...
The Access Evaluation Discovery script implements the [AccessEvaluationDiscoveryType](https://github.com/JanssenProject/jans/blob/main/jans-core/script/src/main/java/io/jans/model/custom/script/type/authzen/AccessEvaluationDiscoveryType.java) interface. This extends methods from the base script type in addition to adding new methods:
...
| Method header                                             | Method description                                                                                                                                                                                                 |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `def modifyResponse(self, responseAsJsonObject, context)` | This method is called after discovery response is ready. This method can modify discovery response. `responseAsJsonObject` is `org.json.JSONObject` `context` is `io.jans.as.server.model.common.ExecutionContext` |
...
1. **Add custom endpoints** - Include additional proprietary endpoints
1. **Add metadata** - Include organization-specific information
1. **Conditional responses** - Modify response based on request context
