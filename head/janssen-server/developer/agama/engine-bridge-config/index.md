...
- `enabled`: A boolean value that specifies if the engine is enabled. To disable the engine, open [TUI](https://docs.jans.io/head/janssen-server/config-guide/config-tools/jans-tui/index.md) and navigate to `Auth Server` > `properties` > `agamaConfiguration`. Then uncheck `enabled` and hit `save`
...
- `maxItemsLoggedInCollections`: When a list or map is [logged](https://docs.jans.io/head/agama/language-reference/#logging) in a flow, only the first few items are included in the output. You can use this property to increase that limit. Default value is `9`
