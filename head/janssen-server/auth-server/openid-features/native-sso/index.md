...
Janssen Server supports [OpenID Connect native SSO mechanism](https://openid.net/specs/openid-connect-native-sso-1_0.html) to enable SSO for mobile applications.
...
Janssen Server also checks if the client has the token exchange grant type enabled. To enable the grant type, use [Janssen Text-based UI(TUI)](https://docs.jans.io/head/janssen-server/config-guide/config-tools/jans-tui/index.md) and enable token exchange grant (`urn:ietf:params:oauth:grant-type:token-exchange`).
...
Janssen Server carries out the processing of token requests as per rules and checks defined [in the specification](https://openid.net/specs/openid-connect-native-sso-1_0.html#name-native-sso-processing-rules).
