...
Use the command line to perform actions from the terminal. Learn how to use Jans CLI [here](https://docs.jans.io/head/janssen-server/config-guide/config-tools/jans-cli/index.md) or jump straight to the [configuration steps](#using-command-line)
...
You can deploy the Agama project in the Janssen Server through the command line using the `post-agama-prj` operation. Here the `agama-project-file` is an archive file that holds the bundled Agama project and follows the [.gama](https://docs.jans.io/head/agama/gama-format/index.md) specification.
...
Take the [sample project configuration](https://docs.jans.io/head/assets/agama/journey-configs.json) and keep it under `/tmp/journey-configs.json`.
...
In Janssen, You can deploy and customize an Agama project using the [Text-Based UI](https://docs.jans.io/head/janssen-server/config-guide/config-tools/jans-tui/index.md) also.
...
Janssen Server Configuration REST API exposes relevant endpoints for managing and configuring Agama projects. Endpoint details are published in the [Swagger document](https://docs.jans.io/head/janssen-server/reference/openapi/index.md).
