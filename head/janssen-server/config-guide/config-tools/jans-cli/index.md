...
- Follow [these instructions](https://docs.jans.io/head/janssen-server/config-guide/config-tools/jans-tui/#1-build-pyz-self-executable-file) and create a self-executable file
...
Janssen CLI tool stores its configuration under `<install-path>/.config` directory. Various settings for CLI tool can be found under this directory, for example [logging levels](https://docs.jans.io/head/contribute/developer-faq/#how-to-enable-debug-logs-for-jans-cli-and-tui-configuration-tools).
...
This file contains multiple individual patch operation. In [Patch Request (schema)](https://docs.jans.io/head/janssen-server/config-guide/config-tools/jans-cli/#patch-request-schema) we explained about each of these keys in the above json file.
