...
Not sure which authorization method to use? See the [decision guide](https://docs.jans.io/head/cedarling/reference/cedarling-authz/#which-authorization-method-should-i-use).
...
- **`CEDARLING_POLICY_STORE_LOCAL_FN`** : Path to local policy store. Cedarling automatically detects the format as one of the following. This property is not supported in WASM due to lack of file-system access.
  ...
  - `.yaml`/`.yml` files → loads as YAML (supported for test suites) *(Note: Legacy `.json` files are no longer supported. Please migrate to the folder-based format or `.cjar` archive.)*
...
- **`CEDARLING_LOG_LEVEL`** : System Log Level [See here](https://docs.jans.io/head/cedarling/reference/cedarling-logs/index.md). Default to `WARN`
