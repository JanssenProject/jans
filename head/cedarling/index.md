...
Developers call the `init` interface on startup of their application, causing the Cedarling to read its [bootstrap properties](https://docs.jans.io/head/cedarling/reference/cedarling-properties/index.md) and load its [policy store](https://docs.jans.io/head/cedarling/reference/cedarling-policy-store/index.md). If configured for JWT validation, the Cedarling will fetch the most recent issuer public keys and metadata.
