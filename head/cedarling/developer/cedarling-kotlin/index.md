...
Please refer to [this document](https://docs.jans.io/head/cedarling/developer/mobile-apps/cedarling-uniffi/index.md) for details on the structs, enums, and functions exposed by UniFFI bindings. This section outlines the process of generating the Kotlin binding for Cedarling using Cedarling UniFFI. The Kotlin binding is then wrapped in a Java class to enable convenient use in Java applications.

## Building from Source

Apart from using the Cedarling binding [using the package manager](https://docs.jans.io/head/cedarling/tutorials/java/#using-the-package-manager), you can also build it from source.
...
- Upload [bootstrap.json](https://docs.jans.io/head/cedarling/uniffi/cedarling-sample-inputs/#bootstrapjson) and policy-store file (with .cjar extension released from the Agama Lab) at `/opt/jans/jetty/jans-auth/custom/static` location of the auth server. The [Asset Screen](https://docs.jans.io/v1.6.0/janssen-server/config-guide/custom-assets-configuration/#asset-screen) can be used to upload assets.
...
- The following java [Update Token](https://docs.jans.io/head/cedarling/uniffi/cedarling-sample-inputs/#sample_cedarling_update_tokenjava) script has been created for calling Cedarling authorization. Enable the script with following Custom Properties:
