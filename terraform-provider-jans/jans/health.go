...
func (c *Client) GetHealthStatus(ctx context.Context) ([]HealthStatus, error) {
	...
	if err := c.get(ctx, "/jans-config-api/api/v1/health", "", "", &healthStatus); err != nil {
...
func (c *Client) GetServerStats(ctx context.Context) (*StatsData, error) {
	...
	var stats StatsData
	if err := c.get(ctx, "/jans-config-api/api/v1/health/server-stat", token, scope, &stats); err != nil {
