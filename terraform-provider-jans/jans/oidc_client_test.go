...
func TestSimpleCreate(t *testing.T) {
	...
	filter := cmp.FilterPath(func(p cmp.Path) bool {
		...
		return attr == "CustomAttributes" || attr == "Dn" || attr == "BaseDn" ||
			...
			attr == "Attributes" || attr == "AuthenticationMethod" ||
			attr == "ResponseTypes" || attr == "GrantTypes" ||
			strings.HasSuffix(attr, "Localized")
	}, cmp.Ignore())
...
func TestOIDCClient(t *testing.T) {
	...
	filter := cmp.FilterPath(func(p cmp.Path) bool {
		...
		return attr == "CustomAttributes" || attr == "ClientSecret" ||
			attr == "ClientName" || attr == "GrantTypes" || attr == "ResponseTypes" ||
			strings.HasSuffix(attr, "Localized")
	}, cmp.Ignore())
