...
func (c *Client) GetScope(ctx context.Context, inum string) (*Scope, error) {
	...
	if err := c.get(ctx, "/jans-config-api/api/v1/scopes/"+inum, token, scope, ret); err != nil {
...
// GetScopesByCreator returns all scopes created by the given creator id.
func (c *Client) GetScopesByCreator(ctx context.Context, creatorId string) ([]Scope, error) {

	if creatorId == "" {
		return nil, fmt.Errorf("creatorId is empty")
	}
...
// GetScopesByType returns all scopes of the given scope type.
func (c *Client) GetScopesByType(ctx context.Context, scopeType string) ([]Scope, error) {

	if scopeType == "" {
		return nil, fmt.Errorf("scopeType is empty")
	}
...
func (c *Client) DeleteScope(ctx context.Context, inum string) error {
	...
	if err := c.delete(ctx, "/jans-config-api/api/v1/scopes/"+inum, token, scope); err != nil {
