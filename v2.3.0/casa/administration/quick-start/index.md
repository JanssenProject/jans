...
- Ensure at least two credentials have been added. In the home page, [turn on](https://docs.jans.io/v2.3.0/casa/user-guide/#turn-2fa-onoff) 2FA
- Log off and log back in. From now on, besides username and password, one credential has to be presented to get access

Click [here](https://docs.jans.io/v2.3.0/casa/administration/2fa-basics/index.md) to learn more about 2FA in Casa.

## Finish configuration

Once you are done with testing, you may use casa as the default authentication method of Janssen Server using [TUI](https://docs.jans.io/v2.3.0/janssen-server/config-guide/config-tools/jans-tui/index.md) to log in users via Casa for all applications the server protects.
...
In cloud-native setup, the admin console can be enabled/disabled by changing the value of `casa.adminEnabled` attribute in `values.yaml` file (and run `helm install` or `helm upgrade`). See examples below:
...
Browse our [catalog of plugins](https://docs.jans.io/v2.3.0/casa/#existing-plugins) to add features and expand Casa!.
