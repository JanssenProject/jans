...
The easiest way to set this up is adding a `VirtualHost` directive associated to a new port, for instance 444. This [snippet](https://github.com/JanssenProject/jans/raw/v2.3.0/jans-casa/plugins/cert-authn/apache/certauthn_vhost_tls1.3.conf) exemplifies a safe way to do so. Note you have to edit accordingly:
...
This is the fastest approach and involves adding a directive inside the existing `VirtualHost` for port 443 in `/etc/apache2/sites-enabled/https_jans.conf`. Copy this [snippet](https://github.com/JanssenProject/jans/raw/v2.3.0/jans-casa/plugins/cert-authn/apache/locationmatch_tls1.2.conf) and paste it just after the closing of the `Location` directive associated to `/jans-casa`.

In this case your cert pickup URL is `https://<your-host-name>/jans-casa/pl/cert-authn/index.zul`.

### Test

Restart Apache (e.g. `systemctl restart apache2`) and in a browser visit the URL configured, e.g. `https://acme.co:444` (TLS 1.3) or `https://acme.co/jans-casa/pl/cert-authn/index.zul` (TLS 1.2). This will display a dialog like [this](https://docs.jans.io/v2.3.0/assets/casa/plugins/cert-authn-browser_dialog.png) listing the certificates imported so far. Do not select anything and close the browser window.
...
- Display a login page as the above including the configured identity providers, if any, in case the [accounts linking](https://docs.jans.io/v2.3.0/casa/plugins/accts-linking/account-linking-index/index.md) plugin is installed
...
1. Download the certificate authentication Agama project archive: `https://github.com/JanssenProject/jans/releases/download/v2.3.0/cert-authn-agama-2.3.0-project.zip`
...
1. Download the plugin jar file `https://github.com/JanssenProject/jans/releases/download/v2.3.0/cert-authn-2.3.0-jar-with-dependencies.jar` and copy to your server's `/opt/jans/jetty/jans-casa/plugins`. Alternatively upload the file using Casa itself: go to `Administration console` > `Casa plugins`
...
1. Download file `https://github.com/JanssenProject/jans/releases/download/v2.3.0/jans-scim-model-2.3.0.jar` and place it in your server under `/opt/jans/jetty/jans-auth/custom/libs`
...
Yes. There is a simple demonstrative [tutorial](https://docs.jans.io/v2.3.0/casa/plugins/cert-authn-tutorial/index.md) available.
