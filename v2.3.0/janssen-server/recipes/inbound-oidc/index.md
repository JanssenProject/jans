...
In this tutorial, we will see how you can integrate any inbound external oauth2 provider authentication in the Janssen server. If you are looking for social login support then check out [passport](https://docs.jans.io/v2.3.0/janssen-server/recipes/passportjs/index.md) module for that.

We will use [interception authentication script](https://docs.jans.io/v2.3.0/script-catalog/person_authentication/oidc/oidc-jans-script.py) which is a [PersonAuthenticationType](https://github.com/JanssenProject/jans/blob/main/jans-core/script/src/main/java/io/jans/model/custom/script/type/auth/PersonAuthenticationType.java) script for whole flow. Currently, It supports `Authorization Code Flow` and `client_secret_post` token endpoint auth method.
...
- The [external oauth2 server authentication script](https://docs.jans.io/v2.3.0/script-catalog/person_authentication/oidc/oidc-jans-script.py)
...
- Edit the `cs.json` file's contents to reflect the addition of the oidc custom script.
  ...
  - Populate the `script` field with `oidc-jans-script.py` script. [Download `oidc-jans-script.py` script from here.](https://docs.jans.io/v2.3.0/script-catalog/person_authentication/oidc/oidc-jans-script.py)
...
- Download [oidc-jans-login.xhtml from here](https://docs.jans.io/v2.3.0/script-catalog/person_authentication/oidc/oidc-jans-login.xhtml)
