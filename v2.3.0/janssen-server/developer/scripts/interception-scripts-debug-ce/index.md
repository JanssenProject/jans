...
1. Log in to CE
1. Run /opt/jans/bin/eclipse.sh Once complete, start the PyDev debug server:
1. Open the Eclipse Debug perspective
1. From the menu: `Pydev` > `Start Debug Server`
...
1. Then instruct jans-auth to load the script from the file system instead of Janssen Server persistence store
...
1. Change script Location type to File using a jans-cli

1. Assume that we're using basic.py, Specify the Script Path location to: /root/eclipse-workspace/basic.py

1. Enable the script using jans-cli Check the following log to verify that jans-auth loaded the script properly: /opt/jans/jetty/jans-auth/logs/jans-auth_script.log. It should look like this:
...
1. Press the "Manual Config" button in the dialog box after opening the Python file

1. Open "PyDev->Interpreters->Jython Interpreters"

1. Click the "New..." button in the right panel. Name it "Jython" and specify the interpreter executable "/opt/jython/jython.jar"

1. Click "OK", then confirm the settings by clicking "OK" again, then "Apply and Close"

1. In the final dialog, confirm the settings by clicking "OK"
...
1. To check if the script works, update the default authentication method to Basic Authentication. Use jans-cli.
...
1. After debugging is finished, resume script execution to transfer execution control back to jans-auth
