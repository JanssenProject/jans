...
Agama engine saves the state of a flow every time an [RRF](https://docs.jans.io/v2.3.0/agama/language-reference/#rrf) or [RFAC](https://docs.jans.io/v2.3.0/agama/language-reference/#rfac) instruction is reached. The *State* can be understood as the set of all variables defined in a flow and their associated values up to a certain point.

Most of times, variables hold basic Agama [values](https://docs.jans.io/v2.3.0/agama/language-reference/#data-types) like strings, booleans, numbers, lists or maps, however, more complex values may appear due to Java `Call`s. The engine does its best to properly serialize these Java objects, nonetheless, this is not always possible. In these cases, the flow crashes and errors will appear on screen as well as in the server logs.
...
- Tweak the engine serialization [rules](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/engine-bridge-config/#serialization-rules) so an alternative type of serialization can be used for this particular object
...
You can include jar files in the `lib` directory of a [project](https://docs.jans.io/v2.3.0/agama/gama-format/#anatomy-of-a-project). This applies only for VM-based installation of Janssen. Onboarding the jar files require a restart of the authentication server.
...
- Supply a [custom log4j](https://docs.jans.io/v2.3.0/janssen-server/auth-server/logging/custom-logs/index.md) descriptor
...
Call method `log` of class `io.jans.agama.engine.script.LogUtils`. This method receives a variable number of arguments as DSL's `Log` does. Thus you can do `LogUtils.log("@w Today is Friday %th", 13)`, as in the logging [examples](https://docs.jans.io/v2.3.0/agama/language-reference/#logging).
...
This topic is covered [here](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/advanced-usages/#localization-and-internationalization).

### How to modify the built-in error pages?

Agama features some error pages out-of-the-box. They can be customized by editing the corresponding files referenced in the [configuration](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/engine-bridge-config/#engine-configuration) doc page.
...
We plan to offer a debugger in the future. In the meantime, you can do `printf`-like debugging using the `Log` instruction. See [Agama logging](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/jans-agama-engine/#logging).

## Miscellaneous

### How to launch a flow?

A flow is launched by issuing an authentication request in a browser as explained in the [launching flows documentation](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/jans-agama-engine/#launching-flows).

### Does flow execution timeout?

Yes. The maximum amount of time an end-user can take to fully complete a flow is driven by the configuration of the authentication server and can be constrained even more in the flow itself. Read about timeouts [here](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/jans-agama-engine/#how-timeouts-work).

### How to prevent launching a flow directly from the browser?

This can be configured in the project's [metadata file](https://docs.jans.io/v2.3.0/agama/gama-format/#metadata).
...
This is to avoid traversing big structures fully. You can increase the value of `maxItemsLoggedInCollections` in the [engine configuration](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/engine-bridge-config/#engine-configuration).
...
*Hint*: methods like `addExact`, `incrementExact`, etc. in `java.lang.Math` may help you to do some arithmetic.
...
See the examples in the Looping section of the [language reference](https://docs.jans.io/v2.3.0/agama/language-reference/#looping).

### How to know the number of iterations carried out by a loop once it has finished?

You can assign this value to a variable at the top of your loop declaration. See the examples in the Looping section of the [language reference](https://docs.jans.io/v2.3.0/agama/language-reference/#looping).

### How to access localization labels from Java code?

In Freemarker, [localization](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/advanced-usages/#localization-and-internationalization) labels are accessed using the `${labels(key, ...)}` notation. The following would be a Java equivalent:
...
No. These two languages are supposed to play roles that should not be mixed, check [here](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/agama-best-practices/#about-flow-design).

### How to run flows from native applications instead of web browsers?

There is a separate doc page covering this aspect [here](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/native-applications/index.md).
