...
The engine and the bridge are two of the components part of the Agama Framework implementation in Janssen. The [engine](https://docs.jans.io/v2.3.0/janssen-server/developer/agama/jans-agama-engine/index.md) is a piece of software in charge of parsing flows written in Agama DSL and put them into action. The "bridge" is a regular jython script that temporarily hands control to the engine when an Agama flow is started, and receives control back once the flow has finished. This script is in charge of completing the authentication process for the user.

By default, the bridge is disabled. To activate it do the following:

- Open [TUI](https://docs.jans.io/v2.3.0/janssen-server/config-guide/config-tools/jans-tui/index.md)
- Navigate to `Scripts` > Search 'agama' > Select the script and hit enter > check `enabled` > `save`

## Engine configuration

Some aspects of the engine are configurable and they are integral part of the Jans authentication server's JSON configuration - specifically the section labeled `agamaConfiguration`. To learn how to perform changes in the server's configuration click [here](https://docs.jans.io/v2.3.0/janssen-server/config-guide/auth-server-config/jans-authorization-server-config/index.md).

The properties of Agama engine configuration are described in the following:

- `enabled`: A boolean value that specifies if the engine is enabled. To disable the engine, open [TUI](https://docs.jans.io/v2.3.0/janssen-server/config-guide/config-tools/jans-tui/index.md) and navigate to `Auth Server` > `properties` > `agamaConfiguration`. Then uncheck `enabled` and hit `save`
...
- `maxItemsLoggedInCollections`: When a list or map is [logged](https://docs.jans.io/v2.3.0/agama/language-reference/#logging) in a flow, only the first few items are included in the output. You can use this property to increase that limit. Default value is `9`
