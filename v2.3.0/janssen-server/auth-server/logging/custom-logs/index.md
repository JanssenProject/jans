...
Note: invalid external `log4j2.xml` can lead to AS start up issues and no logs in [standard log files](https://docs.jans.io/v2.3.0/janssen-server/auth-server/logging/standard-logs/index.md) or otherwise in other log files if such are defined by `log4j2.xml`.
