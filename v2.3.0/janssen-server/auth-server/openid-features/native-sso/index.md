...
Janssen Server also checks if the client has the token exchange grant type enabled. To enable the grant type, use [Janssen Text-based UI(TUI)](https://docs.jans.io/v2.3.0/janssen-server/config-guide/config-tools/jans-tui/index.md) and enable token exchange grant (`urn:ietf:params:oauth:grant-type:token-exchange`).
