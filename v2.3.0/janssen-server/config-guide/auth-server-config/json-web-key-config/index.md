...
In the Janssen Server, you can deploy and manage the JSON Web keys using the command Line. To get the details of Janssen command line operations relevant to JSON Web Key, you can check the operations under `ConfigurationJwkJsonWebKeyJwk` task using the command below:
...
To add a new key, we can use `post-config-jwks-key` operation id. As shown in the [output](#using-command-line) for `--info` command, the `post-config-jwks-key` operation requires data to be sent according to `JSONWebKey` schema.
...
To replace the current JWKS configuration, use the `put-config-jwks` operation. This operation replaces the current set of JWK with the provided set.
...
With `get-jwk-by-kid` operation-id, we can get any specific jwk matched with `kid`. If we know the `kid`, we can simply use the below command:
...
Please read about [patch method](https://docs.jans.io/v2.3.0/janssen-server/config-guide/config-tools/jans-cli/#quick-patch-operations), You can get some idea of how this patch method works to modify particular properties of any task.
...
In Janssen, you can view the JSON web keys using the [Text-Based UI](https://docs.jans.io/v2.3.0/janssen-server/config-guide/config-tools/jans-tui/index.md) also.
...
Navigate to `Auth Server` -> `Keys` to open the JSON Web Key screen as shown in the image below.

- After clicking on `Get keys`, You can view Key's list on this page. You cannot perform any operations in text-based UI.
- Only you can perform operations using the command line.

## Using Configuration REST API

Janssen Server Configuration REST API exposes relevant endpoints for managing and configuring the Json Web key. Endpoint details are published in the [Swagger document](https://docs.jans.io/v2.3.0/janssen-server/reference/openapi/index.md).
