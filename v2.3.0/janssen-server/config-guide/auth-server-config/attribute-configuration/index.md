...
`Attribute` and `User Claims` refer to the same user data fields in the Janssen Server.

# Attribute

User claims are individual pieces of user data, like `uid` or `email`, that are required by applications to identify a user and grant access to protect resources.
...
To get the details of Janssen command line operations relevant to user claims configuration, you can check the operations under `Attribute` task using the command below:
...
The user claims that are available in your Janssen Server can be found by using this operation-ID. If we look at the description below:
...
[Restart](https://docs.jans.io/v2.3.0/janssen-server/vm-ops/jans-command/#restart) the Janssen services below in order to make this change effective:
...
Add and configure the custom claim using `post-attributes` operation-ID. As shown in the [output](#using-command-line) for the `--info` command, the `post-attributes` operation requires data to be sent according to the `JansAttribute` schema.
...
1. Get the [user claim](#get-user-claim-by-inum) and store it into a file for editing. The following command will retrieve the existing user claim in the required schema file.
...
1. Update values as desired. Updates must adhere to the `JansAttribute` schema as mentioned [here](#using-command-line).
...
Close and reopen the TUI. The new custom user claim can now be added to any user from [user management](https://docs.jans.io/v2.3.0/janssen-server/usermgmt/usermgmt-cli-tui/index.md).
...
Janssen Server Configuration REST API exposes relevant endpoints for managing and configuring the user claim. Endpoint details are published in the [Swagger document](https://docs.jans.io/v2.3.0/janssen-server/reference/openapi/index.md).
