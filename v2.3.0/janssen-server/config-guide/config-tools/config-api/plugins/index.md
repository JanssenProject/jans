...
[Jans Config Api](https://github.com/JanssenProject/jans/tree/v2.3.0/jans-config-api) is a REST application that is developed using Weld 4.x (JSR-365) and JAX-RS. Its endpoint can be used to manage configuration and other properties of [Jans Auth Server](https://github.com/JanssenProject/jans/tree/v2.3.0/jans-auth-server).
...
1. Navigate under the plugins/sample folder.
1. This folder contains the sample plugins for reference.
1. `helloworld` folder for example contains the code for a basic plugin with GET endpoint sending `Hello World!` string response.
1. Take the time to explore `helloworld` which is a minimalistic plugin that showcases very basic aspects of plugin development.
...
1. Add the location of plugin jar inside tag with name **extraClasspath** (multiple plugins can be added comma separated) of `/opt/jans/jetty/jans-config-api/webapps/jans-config-api.xml` file.
...
1. Restart `jans-config-api` service.
