...
```
wget https://github.com/JanssenProject/jans/releases/download/v2.3.0/jans-2.3.0-stable.suse16.x86_64.rpm -P ~/
```
...
- Verify the cryptographic signature using cosign (primary verification):
  ...
  - Download the cosign bundle from the [Releases](https://github.com/JanssenProject/jans/releases/latest) page:
    ...
    ```
    wget https://github.com/JanssenProject/jans/releases/download/v2.3.0/jans-suse16-2.3.0-stable.bundle
    ```
...
- Optionally, verify integrity using the published checksum file (secondary check):
  ...
  ```
  echo 'paste-release-sha256sum jans-2.3.0-stable.suse16.x86_64.rpm' | sed 's/^sha256://' >jans-2.3.0-stable.suse16.x86_64.rpm.sha256sum && sha256sum -c jans-2.3.0-stable.suse16.x86_64.rpm.sha256sum
  ```

  Output similar to below should confirm the integrity of the downloaded package.

  ```
  jans-2.3.0-stable.suse16.x86_64.rpm: OK
  ```

- Install the package

```
sudo zypper install ~/jans-2.3.0-stable.suse16.x86_64.rpm
```

Expected zypper prompt during install
...
This is expected. Janssen uses keyless cosign signing (verified in the step above) rather than embedding a GPG signature in the RPM header. Enter `i` to ignore this prompt and proceed with the installation.
