...
```
wget https://raw.githubusercontent.com/JanssenProject/jans/v2.3.0/automation/start_janssen_aio_demo.sh 
...
1. Connect to the TUI using the downloaded `.pyz` file and the extracted credentials. *(Note: add `-noverify` if you are using the self-signed certificates generated by the demo script).*

   ```
   python3 jans-cli-tui-linux-ubuntu-X86-64.pyz --host $FQDN --client-id $TUI_CLIENT_ID --client-secret $TUI_CLIENT_SECRET -noverify
   ```

1. Note that the default `admin` password is `Test1234#`
