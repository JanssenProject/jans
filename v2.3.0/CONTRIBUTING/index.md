...
Janssen project has a [Code of Conduct](https://docs.jans.io/v2.3.0/CODE_OF_CONDUCT/index.md) to which all contributors must adhere, please read it before interacting with the repository or the community in any way.
...
- **Security Vulnerability**: If you identify a security problem, please report it immediately, providing details about the nature, and if applicable, how to reproduce it. If you want to report an issue privately, you can email [security@gluu.org](mailto:security@gluu.org).
...
- Fix an issue: you can help in getting issue fixed in many ways. More often by opening a pull request. In case you are ***first-time*** contributor, then you can start with our [good first issues list](https://github.com/JanssenProject/jans/issues?q=is%3Aissue+is%3Aopen++label%3A%22good+first+issue%22) These are issues where you can easily contribute and community members will guide and support your contribution as always.
...
Here is how we [continously triage](https://docs.jans.io/v2.3.0/governance/triage/index.md) new issues and PRs so that contributors can contribute faster and better.
...
Backport changes are now supported through a [workflow](https://github.com/zeebe-io/backport-action#backport-action) through labels prefixed with `backport/`. For-example to backport changes in a certain PR to version `v1.0.0` a label to that PR matching the version must be added i.e `backport/v1.0.0`. The flow consists of creating a new branch, cherry-picking the changes of the original PR and creating a new PR to merge them.
...
The current version of CLA can be found [here](https://docs.jans.io/v2.3.0/governance/cla/index.md). This is just for your reference. This same CLA will be signed digitally by following the steps below:
...
1. Contributor receives an email with CLA and instructions on how to sign digitally. Follow the instructions and complete the signing process.
...
We often need to reference release numbers in the documentation. For example, [Ubuntu package installation guide](https://docs.jans.io/v2.3.0/janssen-server/install/vm-install/ubuntu/index.md). In this guide, the following command is documented:
...
To avoid this manual, error-prone approach the Janssen Project uses a release marker, `2.3.0` instead of writing actual release numbers in the `head`(latest) documentation branch. So, when there is a need to mention the release number, instead of writing the actual release number, use the release marker. Let's see how to document the above command (at the `head` version) so that it stays up-to-date release after release.

```
wget https://github.com/JanssenProject/jans/releases/download/v2.3.0/jans_2.3.0.ubuntu20.04_amd64.deb -P /tmp
```
...
- Release marker should be used only when contributing to the latest documentation. Not when updating documentation for previous releases.
...
- To make text *italicised*, put an `_` on each side, like this: `_word_`.
...
- PR should include changes in relevant documentation along with code changes. PR is checked by bot to have either one of the following :
