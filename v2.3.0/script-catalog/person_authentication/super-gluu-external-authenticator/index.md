...
1. Select the `FIDO2 server` and complete the post install process.
...
- [Super Gluu interception script](https://docs.jans.io/v2.3.0/script-catalog/person_authentication/super-gluu-external-authenticator/SuperGluuExternalAuthenticator.py) is included in the default Janssen Server distribution

The Super Gluu Interception script can be configured to work in 2 modes: A. Two step Flow (Default mode) B. One step Flow
...
1. Update the custom script and change the enabled attribute to `true` Now Sign-in with Super-Gluu is an available authentication mechanism for your Janssen Server. This means that, using OpenID Connect acr_values, applications can now request Super-Gluu authentication for users.
...
Use this [link](https://github.com/JanssenProject/jans-cli-tui/blob/v2.3.0/docs/cli/cli-default-authentication-method.md) as a reference.
...
The Gluu Server includes a [default public-facing pages for Super Gluu](https://github.com/JanssenProject/jans/blob/v2.3.0/jans-auth-server/server/src/main/webapp/auth/super-gluu/login.xhtml) for enrollment and authentication.

To customize the look and feel of the pages, follow the [customization guide](https://github.com/JanssenProject/jans/blob/v2.3.0/docs/admin/developer/customization/customize-web-pages.md).
...
See the [SCIM documentation](https://docs.jans.io/v2.3.0/janssen-server/usermgmt/usermgmt-scim/#fido-2-devices) on how to manage Super Gluu devices, using the SCIM [protocol].
