...
- The [Sign-in with Apple authentication script](https://docs.jans.io/v2.3.0/script-catalog/person_authentication/apple-external-authenticator/AppleExternalAuthenticator.py) (included in the default Jans-auth Server distribution);
...
1. In the `Register a new identifier` select `Service ID`
1. After filling out the description and identifier name, save the Service ID
1. Now edit the saved Service ID and enable the `Sign in with Apple` checkbox and click `Configure` button
1. Configure the Janssen's server's callback url `https://<your.janssen.server>/postlogin.htm` as a `Website URL`
...
To update this setting in Jans persistence, follow this [link](https://github.com/JanssenProject/jans-cli-tui/blob/v2.3.0/docs/cli/cli-custom-scripts.md#update-an-existing-custom-script)
...
Update the custom script and change the enabled attribute to `true` Now Sign-in with Apple is an available authentication mechanism for your Janssen Server. This means that, using OpenID Connect acr_values, applications can now request Apple authentication for users.
