...
public class CryptoProviderTest extends ConfigurableTest {
    ...
    public void testVerifyES256K() {
        try {
            boolean signatureVerified = cryptoProvider.verifySignature(SIGNING_INPUT, es256KSignature, es256KKey, null, null, SignatureAlgorithm.ES256K);
            assertTrue(signatureVerified);
        } catch (Exception e) {
            fail(e.getMessage(), e);
        }
