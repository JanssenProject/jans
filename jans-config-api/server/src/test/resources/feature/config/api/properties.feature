...
Feature: Verify Auth configuration endpoint
    ...
    And def request_body = (response.loggingLevel == null ? "[ {\"op\":\"add\", \"path\": \"/loggingLevel\", \"value\":"DEBUG" } ]" : "[ {\"op\":\"replace\", \"path\": \"/loggingLevel\", \"value\":"+response.loggingLevel+" } ]")
