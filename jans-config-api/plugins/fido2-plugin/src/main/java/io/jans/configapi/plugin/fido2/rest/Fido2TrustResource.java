...
public class Fido2TrustResource extends BaseResource {
    ...
    public Response getMdsHealth() {
        ...
        try {
            ...
            // The FIDO2 server answers 503 with a diagnostic body when MDS is DOWN, so that the
            // endpoint can be wired straight to a monitor. Fido2Util turns any non-OK upstream status
            // into a WebApplicationException carrying that body as its message; flattening it to 500
            // here would lose both the DOWN signal and the reason, and a monitor pointed at the Config
            // API would disagree with one pointed at the FIDO2 server. Mirror 503 instead.
     ...
     *
     * @return the mirrored response, or {@code null} when the failure was not an upstream 503
     */
