...
class HttpdInstaller(BaseInstaller, SetupUtils):
    ...
    def configure(self):
        ...
        if clone_type == 'rpm' and os_initdaemon == 'init':
            ...
            self.copyFile(Config.apache2_ssl_conf, '/etc/httpd/conf.d/https_gluu.conf')

        if clone_type == 'deb':
            self.copyFile(Config.apache2_ssl_conf, '/etc/apache2/sites-available/https_gluu.conf')
        ...
        error_templates = glob.glob(os.path.join(Config.templateFolder,'apache/*'))
