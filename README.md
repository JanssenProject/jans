...
| **[Jans Cedarling](jans-cedarling)**          | Cedarling is an embeddable stateful Policy Decision Point for authorization requests. In simple terms, the Cedarling returns the answer: should the application allow this action on this resource given these JWT tokens. The Cedarling is written in Rust with bindings to WASM, iOS, Android, and Python. Along with backend, Cedarling can run in a browser using the WASM engine.                                                                | ![Incubating](https://img.shields.io/badge/Incubating-%23f79307) |
