%define name1 oxd-server-4.0.0
Name:           oxd-server-4.0.0
Version:        1
Release:        1.centos6
...
Source0:        %{name}.tar.gz
Source1:        oxd-server-4.0.0.init.d
Source2:        oxd-https-extension
Source3:        oxd-server-default
BuildArch:      noarch
Conflicts:      oxd-server-4.0.0
...
install -m 644 %SOURCE3 %{buildroot}/etc/default/oxd-server-4.0.0
...
cp -a debian/oxd-server-default %{buildroot}/etc/default/oxd-server-4.0.0

%clean
rm -rf $RPM_BUILD_ROOT

%post
chkconfig --add oxd-server-4.0.0
...
chown root:root /etc/default/oxd-server-4.0.0 2>&1
chown jetty:jetty -R /opt/oxd-server 2>&1
chmod 644 /etc/default/oxd-server-4.0.0 2>&1
