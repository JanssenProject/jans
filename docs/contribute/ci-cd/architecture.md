...
Overview of the GitHub Actions workflows under `.github/workflows/` and how they
chain. See [Workflow Reference](workflows.md) for a per-workflow table and
[Security Scanning](security-scanning.md) for the scan and pen-test topology.
...
!!! note "Renaming caution"
    ...
    workflow `name:`. When a `name:` changes, update the listeners and the
    repository's required-check settings in the same cutover.

## Ephemeral environments

- `test-integration.yml` — builds AIO from source on an ephemeral DigitalOcean droplet.
- `test-terraform-provider.yml` and `scan-pentest.yml` — bring up the prebuilt AIO
  compose stack (consul+vault+traefik+DB+AIO) on the runner.
