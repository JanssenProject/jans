...
| `CEDARLING_LOCK_TRANSPORT` | The transport type to use for communication with the Lock Server. The gRPC transport requires the `grpc` Cargo feature and is not available for WASM builds. | `rest`, `grpc` | `rest` |
| `CEDARLING_LOCK_GRPC_ENDPOINT` | Connection string used to connect to the Lock Server via gRPC. This is required if `CEDARLING_LOCK_TRANSPORT` is set to `grpc`. Not applicable in WASM builds. | String | `""` |
