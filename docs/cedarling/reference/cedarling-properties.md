...
- **`CEDARLING_JWT_STATUS_LIST_REFRESH_INTERVAL_FALLBACK`** : Fallback Status List JWT refresh interval, in seconds, used only when the Status List JWT fetched from the issuer has no `ttl` claim. When the JWT carries a `ttl` value, that value is always respected (per the IETF `oauth-status-list` spec) and this fallback is ignored. A value of `0` or an unset variable resolves to the built-in default (`300` seconds) so the status list cannot silently go stale forever. Non-zero values below `5` are clamped to `5`.
