...
- **`CEDARLING_CUSTOM_TOKEN_PROCESSOR_TIMEOUT_MILLIS`** : Timeout in milliseconds applied to the custom token processor call for **non-JWT** custom tokens. `0` (default) disables the timeout. The deadline is enforced on native (non-WASM) targets only; on WASM the processor runs to completion regardless of this value.
