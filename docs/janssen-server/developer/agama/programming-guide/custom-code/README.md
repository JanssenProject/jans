...
The root of the (Java) package hierarchy is located in the `lib` folder of a project; `lib` is part of the [gama format](https://docs.jans.io/stable/agama/gama-format/). For the current example, it is the directory located [here](./project/lib) and includes a couple of Java classes belonging to package `com.acme.agama.survey`.
...
There is a single flow: [`com.acme.workaday.club_survey`](./project/code/com.acme.workaday.club_survey.flow). 

### Configuration

Note the use of `Configs` directive. Here is an example of how Agama variable `conf` may look like (see [`project.json`](./project/project.json)):
...
`showIntro` determines if the "welcome page" should be displayed, while `pages` is the list of (question) HTML pages to display. Every page is associated an identifier and a physical location. The templates for this example can be found [here](./project/web/q). Every page features a single answer.
...
The flow file starts with self-explanatory code that will render the page [`intro.ftlh`](./project/web/intro.ftlh) if necessary. 
...
Important remarks about methods in [`AnswersHandler`](./project/lib/com/acme/agama/survey/AnswersHandler.java):
...
Rendering is carried out using template [`survey.ftlh`](./project/web/survey.ftlh) which is injected with the following:
...
Before terminating, the page [`outro.ftlh`](./project/web/outro.ftlh) is shown. It displays a static "thank you" message and a submission button.
