...
Before fetching the content of a `Sector Identifier URI`, the Janssen Server resolves its host and rejects the URI if
it resolves to a private, loopback, or link-local IP address (e.g. `127.0.0.1`, `10.0.0.0/8`, `192.168.0.0/16`,
`169.254.0.0/16`), or if the host cannot be resolved at all. This prevents Server-Side Request Forgery (SSRF) attacks,
where a registered `Sector Identifier URI` is used to make the Janssen Server issue requests to internal-only network
resources.

The `requestUriBlockList` configuration property can additionally be used to explicitly block specific hosts or URL
patterns from being used as a `Sector Identifier URI`, regardless of what they resolve to.
