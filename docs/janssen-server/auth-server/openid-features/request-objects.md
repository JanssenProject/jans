...
Janssen Server supports Request Objects at the [authorization endpoint](../endpoints/authorization.md) using the `request` and `request_uri` parameters. The same mechanism is also used with [PAR](../oauth-features/par.md) and CIBA flows.
...
The `request_uri_parameter_supported` discovery value indicates whether the OP supports this parameter. If Janssen has [requestUriParameterSupported](../../reference/json/properties/janssenauthserver-properties.md#requesturiparametersupported) set to `false` and the RP sends `request_uri`, Janssen returns `request_uri_not_supported`.
...
When [requestUriHashVerificationEnabled](../../reference/json/properties/janssenauthserver-properties.md#requesturihashverificationenabled) is enabled, the URL may include a fragment with the base64url-encoded SHA-256 hash of the JWT content (per RFC 9101). Janssen verifies the hash after fetching the resource.
...
Janssen additionally validates `request_uri` against the client's pre-registered `request_uris` (when configured) and against [requestUriBlockList](../../reference/json/properties/janssenauthserver-properties.md#requesturiblocklist) (for example to block `localhost`).
...
- When [requireRequestObjectEncryption](../../reference/json/properties/janssenauthserver-properties.md#requirerequestobjectencryption) is `true`, the Request Object must be a five-segment JWE.
- When [forceSignedRequestObject](../../reference/json/properties/janssenauthserver-properties.md#forcesignedrequestobject) is `true`, every authorization request must include `request` or `request_uri`, and unsigned Request Objects (including `none` or unrecognized algorithms) are rejected.
...
| `request_not_supported` | The OP does not support the `request` parameter ([requestParameterSupported](../../reference/json/properties/janssenauthserver-properties.md#requestparametersupported) is `false`). |
| `request_uri_not_supported` | The OP does not support the `request_uri` parameter ([requestUriParameterSupported](../../reference/json/properties/janssenauthserver-properties.md#requesturiparametersupported) is `false`). |
| `invalid_scope` | For example, when OIDC scopes are requested from the JWT but `openid` is missing from the URL `scope` parameter. |

Other standard authorization errors (`interaction_required`, `login_required`, `consent_required`, etc.) apply as documented on the [authorization endpoint](../endpoints/authorization.md).
...
| -------- | ----------- |
| [requestParameterSupported](../../reference/json/properties/janssenauthserver-properties.md#requestparametersupported) | Enable or disable the `request` parameter |
| [requestUriParameterSupported](../../reference/json/properties/janssenauthserver-properties.md#requesturiparametersupported) | Enable or disable the `request_uri` parameter |
| [requestObjectSigningAlgValuesSupported](../../reference/json/properties/janssenauthserver-properties.md#requestobjectsigningalgvaluessupported) | JWS algorithms accepted for Request Objects |
| [requestObjectEncryptionAlgValuesSupported](../../reference/json/properties/janssenauthserver-properties.md#requestobjectencryptionalgvaluessupported) | JWE `alg` values accepted for Request Objects |
| [requestObjectEncryptionEncValuesSupported](../../reference/json/properties/janssenauthserver-properties.md#requestobjectencryptionencvaluessupported) | JWE `enc` values accepted for Request Objects |
| [forceSignedRequestObject](../../reference/json/properties/janssenauthserver-properties.md#forcesignedrequestobject) | Require a signed Request Object on every authorization request |
| [requireRequestObjectEncryption](../../reference/json/properties/janssenauthserver-properties.md#requirerequestobjectencryption) | Require Request Objects to be encrypted (JWE) |
| [requireRequestUriRegistration](../../reference/json/properties/janssenauthserver-properties.md#requirerequesturiregistration) | Require `request_uri` values to be pre-registered on the client |
| [requestUriBlockList](../../reference/json/properties/janssenauthserver-properties.md#requesturiblocklist) | Block patterns for disallowed `request_uri` hosts/paths |
| [requestUriHashVerificationEnabled](../../reference/json/properties/janssenauthserver-properties.md#requesturihashverificationenabled) | Verify SHA-256 hash in `request_uri` fragment |
| [staticDecryptionKid](../../reference/json/properties/janssenauthserver-properties.md#staticdecryptionkid) | Key ID for decrypting JWE when `kid` is absent in the JWE header |
...
- Combine with [fapiCompatibility](../../reference/json/properties/janssenauthserver-properties.md#fapicompatibility) and [requirePar](../../reference/json/properties/janssenauthserver-properties.md#requirepar) for FAPI-style deployments.

When [fapiCompatibility](../../reference/json/properties/janssenauthserver-properties.md#fapicompatibility) is enabled, Janssen enforces additional checks on Request Objects (for example `aud`, `exp`, `nbf`, `nonce`, `scope`, and `redirect_uri` in the JWT, and restrictions on `RS256` / `none` for JWS).

### Client metadata

Configure per-client Request Object behavior via TUI (`Auth Server` -> `Clients`), [Dynamic Client Registration](../endpoints/client-registration.md), or client management APIs.
...
- [Security best practices](../../planning/security-best-practices.md)
