...
Using Agama administrators can delegate authorization to external services like social sites. A typical scenario is users clicking on a "Sign in with..." button for authentication to take place at a given 3rd party and finally getting access to a target application.
...
Note the scope here is limited: no session is created in the Janssen server for the user - this occurs later in a [main](#main-flow) parent flow.
...
This is where the actual process of inbound identity occurs. This flow is already [implemented](https://github.com/JanssenProject/jans/raw/main/docs/script-catalog/agama/inboundID/io.jans.inbound.ExternalSiteLogin) and ready to use. The following is a summary of the steps involved:
...
1. The flow associated to the selected provider is launched. Actually, if the main flow was passed a provider identifier as parameter, step 1 is skipped and the associated flow is launched directly  

1. If the flow triggered in step 2 did not finish successfully, the main flow finishes likewise. Else, the profile data returned is transformed using the applicable attribute mapping for the provider. More on attribute mappings [here](#attribute-mappings)

1. If the resulting profile data lacks a value for e-mail, and the provider was configured to require it, a form is displayed where the user is can enter his e-mail address

1. User provisioning takes place, in other words, a local user entry is created in the database of the Janssen server. If an entry already exists, it is updated as long as the provider is configured to allow so
...
- Ensure Agama engine is [enabled](https://jans.io/docs/admin/developer/agama/quick-start/#enable-the-engine) and download the flow [source](https://github.com/JanssenProject/jans/raw/main/docs/script-catalog/agama/inboundID/io.jans.inbound.ExternalSiteLogin) file
...
- Login to Facebook and [register](https://developers.facebook.com/docs/development/register) as developer. Create an application and enable OAuth2 capabilities. Add the url `https://<your-host>/jans-auth/fl/callback` as a valid redirect URI for your app. Finally grab the app Id and secret
...
Later, we'll dive into the meaning of the [configuration parameters](#provider-flow-configurations) set in the json file.
...
Later, we'll dive into the meaning of the [configuration parameters](#main-flow-configurations) set in the json file.
...
|`logoImg`|(Relative) path to the logo image (will be shown in the selector page)|Yes|
...
|`skipProfileUpdate`|Determines if profile data should be updated for a user if an entry already exists locally for him. Default value is `false`|No|
...
Configurations for this kind of flows don't have to adhere to any specific structure. Developers are free to choose what fits best for their needs. Also note provider flows **must not** receive any inputs: the main flow won't pass any arguments when triggering them.

!!! Important
    Design your flows so there is no use of `Inputs` but `Configs` directive in the [header](https://jans.io/docs/admin/developer/agama/dsl-full/#header-basics) 

In practice many identity providers adhere to the OAuth2 `code` grant, so you can use the structure represented by [this](https://github.com/JanssenProject/jans/blob/main/agama/inboundID/src/main/java/io/jans/inbound/oauth2/OAuthParams.java) Java class for the purpose. Particularly, the already implemented flows (like Facebook) use it for their configuration.
...
This is the process through which the raw user profile data received by an identity provider is transformed into an object suitable for being stored in the Janssen user database. Here, developers have the opportunity to "map" or "transform" the incoming data to one compatible with the data types, formats, and names required by the database.
...
None of this attributes exist in Janssen. The database adheres to LDAP naming - conformant names would be `uid`, `mail`, `sn`, `givenName`. Also, let's assume you want to set `displayName` to a string composed by the first and last name separated by a white space. Writing a mapping is required.
...
Note property `mappingClassField` of every provider defined in the [main flow](#main-flow) points to the fully qualified name of a mapping. Some important considerations:
...
While working on a mapping, having to pack the class in a jar file and restarting the server every time a modification is made can be a big burden. To avoid this you can upload the source (java) file to the scripts directory of Agama and leverage hot reloading as outlined [here](https://jans.io/docs/admin/developer/agama/java-classpath/). A "template" for quickly start writing a mapping is provided [here](https://github.com/JanssenProject/jans/raw/main/agama/inboundID/CustomMappings.java.txt). Save with `.java` extension only, edit the body of the lambda expression, upload to the server, and update the main flow as follows:   
